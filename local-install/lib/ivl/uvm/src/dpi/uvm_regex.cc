...
const char * uvm_re_deglobbed(const char *glob, unsigned char with_brackets)
{
  ...
  if(glob == NULL) {
    snprintf(buffer, UVM_REGEX_MAX_LENGTH+4,
             "uvm_glob_to_re called with glob=NULL");
    return NULL;
  }
  ...
  if (glob[0] == uvm_re_bracket_char && glob[glob_len-1] == uvm_re_bracket_char) {
    if (glob_len > UVM_REGEX_MAX_LENGTH) {
      snprintf(buffer, UVM_REGEX_MAX_LENGTH+4,
               "uvm_glob_to_re() glob exceeds max length (%0d)",
    ...
    if (with_brackets) {
      ...
      if (glob_len > UVM_REGEX_MAX_LENGTH) {
        snprintf(buffer, UVM_REGEX_MAX_LENGTH+4,
                 "uvm_glob_to_re glob expansion exceeds max length (%0d)",
                 UVM_REGEX_MAX_LENGTH);
        return NULL;
      }
    ...
    for(p = glob; *p; p++) {
      ...
      switch(*p)
        ...
        case '+':
          ...
          if (glob_len > UVM_REGEX_MAX_LENGTH) {
            snprintf(buffer, UVM_REGEX_MAX_LENGTH+4,
                     "uvm_glob_to_re() expansion exceeds max length(%0d)",
                     UVM_REGEX_MAX_LENGTH);
            return NULL;
          }
          buffer[iter++] = '.';
...
regex_ptr uvm_re_comp(const char* re, unsigned char deglob)
{
  ...
  if (deglob) {
    ...
    if (re_len > UVM_REGEX_MAX_LENGTH) {
      snprintf(buffer, UVM_REGEX_MAX_LENGTH+4,
               "uvm_re_comp() re exceeds max length (%0d)",
               UVM_REGEX_MAX_LENGTH);
      return NULL;
    }
    else {
  ...
  if (rexp == NULL) {
    snprintf(buffer, UVM_REGEX_MAX_LENGTH+4,
             "uvm_re_comp: internal memory allocation error");
    return NULL;
  }
...
int uvm_re_exec(regex_ptr rexp, const char * str) {
  // Safety check.
  if (rexp == NULL) {
    snprintf(uvm_re_buffer(), UVM_REGEX_MAX_LENGTH+4,
             "uvm_re_exec: NULL rexp");
    return REG_NOMATCH;
  }
  if (str == NULL) {
    snprintf(uvm_re_buffer(), UVM_REGEX_MAX_LENGTH+4,
             "uvm_re_exec: NULL str");
    return REG_NOMATCH;
  }
