...
Commits since full UVM: 7 (ivl.def Windows exports; M9-11 expect;
...
metadata — VPI/metadata only, covered by VPI 87/87, vif 14/14,
smoke 14/14, full ivtest fail-list byte-identical)
Highest risk change since last full run: LOW-MEDIUM (SVA lowering
paths in pform.cc + VPI object/array/coverage/modport paths).
NOTE: the NEXT stable commit reaches the 8-commit full-UVM trigger —
run the 4x53 batches before or with that commit.
