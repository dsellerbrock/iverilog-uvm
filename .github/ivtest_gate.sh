...
if ! python3 "$ROOT/tests/vvp_runtime/run_indexed_darray_slice.py"; then
    status=1
fi
...
if [ -z "$runtime_vvp" ] || [ -z "$runtime_iverilog" ] \
   ...
   || ! VVP="$runtime_vvp" \
        ...
        >> "$WORK/vvp-runtime.log" 2>&1 \
   || ! IVERILOG="$runtime_iverilog" VVP="$runtime_vvp" \
        bash tests/vvp_runtime/run_string_repeat_ir_compat.sh \
        >> "$WORK/vvp-runtime.log" 2>&1 \
   || ! VVP="$runtime_vvp" \
   ...
   || ! VVP="$runtime_vvp" \
        ...
        >> "$WORK/vvp-runtime.log" 2>&1 \
   || ! VVP="$runtime_vvp" \
        bash tests/vvp_runtime/run_mixed_wait_malformed.sh \
        >> "$WORK/vvp-runtime.log" 2>&1 \
   || ! VVP="$runtime_vvp" \
