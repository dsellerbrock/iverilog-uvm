...
module long_sequence_checker_1001(input logic clk, start, keep);
...
module sv_assert_long_register_1001;
  logic clk = 0, start = 1, good_keep = 1;
  long_sequence_checker_1001 good(clk, start, good_keep);
  initial begin
    for (integer i = 0; i < 1001; i++) begin
      #5 clk = 1;
      #1 clk = 0;
      if (i == 0) start = 0;
    end
    #1;
    if (good.passes != 1001 || good.fails != 0)
      $fatal(1, "long 1001 good=%0d/%0d", good.passes, good.fails);
    $display("PASSED");
    $finish(0);
  end
endmodule
