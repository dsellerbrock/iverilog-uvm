// An out-of-range selected receiver must fail loudly under compatibility mode.
module top;
  class reg_c;
    const int offset;
  ...
  class wrapper_c;
    covergroup cg(block_c ral) with function sample(int value);
      cp: coverpoint value { bins selected = {ral.err[2].get_offset()}; }
    endgroup
  ...
  initial begin
    ...
    $fatal(1, "out-of-range receiver did not stop construction");
  end
endmodule
