...
// matter how long it outlives the call. Real, string and container
// (dynamic array, queue, fixed array) formals still take the copy pair
// -- their reads and writes go through a type-specific opcode a bound
// formal cannot answer -- and are exercised below as controls to pin
// that the still-open gap for them is unchanged, not worse.
...
module main;
  ...
  initial begin
    ...
    if (gr == 3.5)
      $display("NOTE: ref real now also survives a detached branch (gap closed; update this control).");
    if (gq.size() != 0)
