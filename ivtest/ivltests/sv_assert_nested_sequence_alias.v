...
module check #(parameter HI=3)(input clk,a,b,c,d,dis);
 ...
 sequence Left; Pair(a,b); endsequence
 sequence Right; Pair(c,d); endsequence
 sequence OuterLeft; Left; endsequence
 sequence OuterRight; Right; endsequence
 flat: assert property (@(posedge clk) disable iff(dis) a ##2 b |-> ##[0:HI] 1'b1) fp++; else ff++;
 ors: assert property (@(posedge clk) disable iff(dis) OuterLeft or OuterRight |-> ##[0:HI] 1'b1) op++; else ofail++;
 ands: assert property (@(posedge clk) disable iff(dis) OuterLeft and OuterRight |-> ##[0:HI] 1'b1) ap++; else af++;
endmodule
module sv_assert_nested_sequence_alias;
