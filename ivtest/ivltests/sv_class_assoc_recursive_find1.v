module test;
  ...
  class phase_base;
    ...
    function new(string name = "phase",
                 ...
                 phase_base parent = null);
      m_name = name;
      m_phase_type = phase_type;
      m_parent = parent;
      if (parent == null && (phase_type == PH_SCHEDULE || phase_type == PH_DOMAIN)) begin
        phase_base end_node;
        end_node = new({name, "_end"}, PH_TERMINAL, this);
        m_successors[end_node] = 1;
      end
    endfunction
    ...
    function phase_base get_schedule(bit hier = 0);
      phase_base sched;
      sched = this;
      if (hier) begin
        while (sched.m_parent != null
               && (sched.m_parent.get_phase_type() == PH_SCHEDULE)) begin
          sched = sched.m_parent;
        end
      end
      if (sched.m_phase_type == PH_SCHEDULE)
        return sched;
      if (sched.m_phase_type == PH_NODE) begin
        if (m_parent != null && m_parent.m_phase_type != PH_DOMAIN)
          return m_parent;
      end
      return null;
    endfunction
    ...
    function phase_base m_find_successor(phase_base phase,
                                         bit stay_in_scope = 1,
                                         phase_base orig_phase = null);
      phase_base found;
      if (phase == null)
        return null;
  ...
  class domain_t extends phase_base;
    ...
    function void add_imp(phase_base phase);
      phase_base new_node;
      new_node = new(phase.m_name, PH_NODE, this);
      new_node.m_imp = phase;
      m_successors[new_node] = 1;
    endfunction
  endclass

  class imp_t extends phase_base;
    function new(string name = "imp");
