...
module main;
  ...
  task automatic watch_bit(input bit initial_value, input bit expression_mode);
    logic [3:0] value;
    time origin;
    origin = $time;
    ...
    begin
      ...
      fork
        begin
          if (expression_mode) @(posedge (value[2] | 1'b0));
          else @(posedge value[2]);
  ...
  initial begin
    ...
    repeat (3) begin
      fork watch_bit(0, 0); watch_bit(1, 0); join
      check_times();
      fork watch_bit(0, 1); watch_bit(1, 1); join
      check_times();
