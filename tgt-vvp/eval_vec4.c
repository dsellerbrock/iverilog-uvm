	 ...
	 * data-type assertion aborts. */
      if (expr_is_dynarray_container_(subexpr) &&
	    ...
	    assert(base);
	       ...
	       * associative array (Mv/Mr/MS), the index is a key (string
      ...
      if (ivl_expr_value(subexpr)==IVL_VT_DARRAY) {
	    ...
	    if (net_type && ivl_type_queue_assoc_compat(net_type)
            ...
            /* M14: integer(vec4)-keyed associative array read. This bare-
               signal branch previously handled only string and object
               keys and fell through to the POSITIONAL darray load below
               for integral keys, so a module-static `int m[int]` (which
               is a bare signal, unlike a class-member assoc) stored its
               values via %aa/store but read them via %load/dar/vec4 and
               always read the default — a silent value loss. An
               assoc-compat container whose key is neither string nor
               object is keyed by an integral value: load it as such. */
            if (net_type && ivl_type_queue_assoc_compat(net_type)) {
                  draw_eval_vec4(base);
                  fprintf(vvp_out, "    %%load/obj v%p_0;\n", sig);
                  fprintf(vvp_out, "    %%aa/load/v/v %u;\n", wid);
...
static void draw_unary_inc_dec(ivl_expr_t sub, bool incr, bool pre)
{
      ...
      if (is_property) {
	    ...
	    fprintf(vvp_out, "    %%flag_mov %d, 4; preserve string character selector\n",
	            select_flag);
	    if (signal_is_return_value(sig))
	          fprintf(vvp_out, "    %%retload/str 0; increment string return character\n");
	    else
	          fprintf(vvp_out, "    %%load/str v%p_0; increment string character\n", sig);
      ...
      if (pre) {
	    ...
	    if (is_property) {
		  ...
		  fprintf(vvp_out, "    %%flag_mov 4, %d; restore string character selector\n",
		          select_flag);
		  if (signal_is_return_value(sig))
			fprintf(vvp_out, "    %%putc/ret/vec4 0, %d; increment string return character\n",
			        select_index);
		  else
	    ...
	    if (is_property) {
		  ...
		  fprintf(vvp_out, "    %%flag_mov 4, %d; restore string character selector\n",
		          select_flag);
		  if (signal_is_return_value(sig))
			fprintf(vvp_out, "    %%putc/ret/vec4 0, %d; decrement string return character\n",
			        select_index);
		  else
