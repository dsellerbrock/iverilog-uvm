...
extern void compile_class_covgrp_cross_term(uint64_t family, uint64_t dim,
					    ...
					    uint64_t source_id,
					    uint64_t source_aux);
extern void compile_class_covgrp_cross_bin(uint64_t family, uint64_t kind,
					   uint64_t target, char*name,
					   char*select_ir);
extern void compile_class_covgrp_item(uint64_t at_least, uint64_t weight,
			      uint64_t is_cross, char*name = 0,
			      char*weight_ir = 0,
			      uint64_t guardsrc = 0);
extern void compile_class_covgrp_options(uint64_t merge, uint64_t weight,
      uint64_t weight_prop, uint64_t get_inst, uint64_t get_inst_prop);
extern void compile_class_covgrp_type_weight(uint64_t weight);
extern void compile_class_covgrp_item_type_weight(uint64_t item_idx, uint64_t weight);
