...
typedef enum ivl_lifetime_e ENUM_UNSIGNED_INT {
      IVL_VLT_INHERITED = 0,
      IVL_VLT_STATIC = 1,
      IVL_VLT_AUTOMATIC = 2
} ivl_lifetime_t;
 ...
 *
 * ivl_event_lifetime
 *    Return the declaration's explicit lifetime override. An unqualified
 *    event reports IVL_VLT_INHERITED; the target combines that with the
 *    enclosing scope lifetime when selecting storage.
 *
 * ivl_event_scope
...
extern ivl_lifetime_t ivl_event_lifetime(ivl_event_t net);
...
extern unsigned    ivl_event_observer_edge(ivl_event_t net);
 ...
 *
 * For nested property expressions (e.g. "foo.bar.property"), the
 * ivl_expr_signal function returns null, and the ivl_expr_oper2
 * function returns the base expression (e.g. the property expression
 * for "foo.bar") from which this property is accessed.
 *
...
extern ivl_scope_t ivl_expr_def(ivl_expr_t net);
  /* IVL_EX_UFUNC */
extern unsigned ivl_expr_is_super_call(ivl_expr_t net);
  /* IVL_EX_DELAY */
...
extern ivl_expr_t  ivl_expr_oper1(ivl_expr_t net);
  /* IVL_EX_BINARY IVL_EX_NEW IVL_EX_PROPERTY IVL_EX_TERNARY */
 ...
 *
 * ivl_signal_lifetime
 *    Signals declared inside subroutines or procedural blocks can
 *    carry an explicit lifetime override (`static` or `automatic`)
 *    that differs from the containing scope default.
 *
 ...
 *
 * ivl_type_is_shortreal
 *    Returns TRUE when the type is shortreal. Both shortreal and real use
 *    IVL_VT_REAL internally, but DPI maps them to distinct C float and
 *    double ABIs.
 *
 * ivl_type_signed
...
extern int ivl_type_is_shortreal(ivl_type_t net);
...
extern unsigned ivl_type_covgrp_bin_trans_repeat(ivl_type_t net, int idx);
extern uint64_t ivl_type_covgrp_bin_trans_min(ivl_type_t net, int idx);
extern uint64_t ivl_type_covgrp_bin_trans_max(ivl_type_t net, int idx);
extern unsigned ivl_type_covgrp_bin_trans_alt(ivl_type_t net, int idx);
extern unsigned ivl_type_covgrp_bin_trans_alt_count(ivl_type_t net, int idx);
extern unsigned ivl_type_covgrp_bin_trans_family(ivl_type_t net, int idx);
extern uint64_t ivl_type_covgrp_bin_trans_base(ivl_type_t net, int idx);
extern unsigned ivl_type_covgrp_bin_guard(ivl_type_t net, int idx);
