...
The [OpenTitan selected-bit NBA and task-error candidate](session_logs/2026-09-23_opentitan_nba_codegen_focus.json) at source/test commit `a1c94078b` passes focused paired-edition checks and all required local compiler, ivtest, JSON, SVA NFA, and real-DPI UVM gates. The pinned SPI Host source compiles without target errors using `-gcommercial-unsafe`, but four unresolved constraint indices and one dropped queue-pop expression still prevent a semantic compile or DV-pass claim. Its correctly configured fixed-seed smoke passes the earlier time-zero regex barrier and aborts on a VIF event-relay runtime form at 3,673,793 ps. The target-specific [release overlay profiles](release_overlays/README.md) define the pinned application sources, patch selection, and observed run options.
