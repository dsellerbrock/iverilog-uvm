...
The opt-in `-gcommercial-unsafe` mode merged in [PR326](https://github.com/dsellerbrock/iverilog-uvm/pull/326) at `fa5ae6da5` (source commit `9111127532c71fc4eeaf1bab68685ade3c61296f`). Its focused checks are in [the revision-scoped record](session_logs/2026-09-23_commercial_unsafe_flag.json); no full suite ran on the flag revision before merge. The OpenTitan SPI compile clears the strict type errors with the flag but still reports unsupported nonblocking property assignment codegen errors; OpenTitan DV has not passed. The target-specific [release overlay profiles](release_overlays/README.md) define the pinned application sources, patch selection, and observed run options without changing those qualification limits.
