...
The [class-property queue-pop candidate](session_logs/2026-09-23_opentitan_queue_pop_focus.json) at source/test commit `a0e45a3c7` has passing paired focused tests and a pinned SPI Host compile with no queue-pop drop. Four unresolved constraint-index warnings still prevent a semantic SPI compile or DV-pass claim. Broad qualification and CI are pending; the branch depends on PR328.
