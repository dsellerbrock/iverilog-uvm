...
The [Caliptra long-sequence register fix](session_logs/2026-09-23_caliptra_long_sequence_register_focus.json) at `4ff0581df` passed paired 999/1000/1001-step executable checks, the focused negative and dual-engine SVA suites, and `make check` before integration. The clean pinned PM formal file lost duplicate `b999` errors but still had 16 late-clock diagnostics on that earlier branch. PR334 is now integrated locally; its combined compiler tree and pinned PM filelist are being rechecked. CI, full DV, and formal proof remain open.

The [Caliptra late-default-clocking candidate](session_logs/2026-09-23_caliptra_late_default_clocking_focus.json) records paired 2017/2023 checker execution and rejection boundaries on source/test commit `342226068`; the pinned clean ECC formal compile moves past five false clock diagnostics but still fails at a separate `DSA_NOP` binding error. A [one-token, test-specific released-source overlay](session_logs/2026-09-23_caliptra_ecc_nop_overlay.json) makes that filelist compile in a disposable copy; no formal proof or DV runtime is claimed. [PR334](https://github.com/dsellerbrock/iverilog-uvm/pull/334) is open and being integrated with merged PR332/333; merged-tree validation and fresh exact-head CI remain pending.
