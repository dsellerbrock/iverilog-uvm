...
The [Caliptra consequent-repetition baseline](session_logs/2026-09-23_caliptra_param_consequent_repeat_baseline.json) records two dropped properties in a clean pinned v2.1.2 formal file under both IEEE modes. A minimal parameter-bound reducer fails while a literal-bound control compiles. This is the selected next SVA implementation blocker; it has no assertion runtime or full Caliptra formal qualification yet. PRs 328 and 329 remain separate pending integration.
...
The earlier [OpenTitan selected-bit NBA and task-error candidate](session_logs/2026-09-23_opentitan_nba_codegen_focus.json) at source/test commit `a1c94078b` passes focused paired-edition checks and all required local compiler, ivtest, JSON, SVA NFA, and real-DPI UVM gates. Before the queue-pop change above, the pinned SPI Host source compiled without target errors using `-gcommercial-unsafe` but retained four unresolved constraint indices and one dropped queue-pop expression. Its correctly configured fixed-seed smoke passed the earlier time-zero regex barrier and aborted on a VIF event-relay runtime form at 3,673,793 ps. The target-specific [release overlay profiles](release_overlays/README.md) define the pinned application sources, patch selection, and observed run options.
