...
The [Caliptra consequent-repetition candidate](session_logs/2026-09-23_caliptra_param_consequent_repeat_focus.json) records paired-edition executable assertion checks and a pinned formal-file compile with no dropped repeat properties; the [baseline](session_logs/2026-09-23_caliptra_param_consequent_repeat_baseline.json) preserves the original failure. Broader local validation and publication are tracked in the candidate record. The standalone bind-target error and full Caliptra DV/formal qualification remain separate. PR328 merged; PR329 remains open pending current-head CI.
