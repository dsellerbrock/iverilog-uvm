...
The [class-property queue-pop candidate](session_logs/2026-09-23_opentitan_queue_pop_focus.json) at `a0e45a3c7` has passing paired focused tests and a pinned SPI Host compile with no queue-pop drop. A [review boundary follow-up](session_logs/2026-09-23_queue_pop_assoc_boundary.json) at `3b6d1986f` rejects queue-only pops on associative arrays while preserving queues stored inside them. Four unresolved constraint-index warnings still prevent a semantic SPI compile or DV-pass claim. Broad qualification and CI are pending; the branch depends on PR328.
