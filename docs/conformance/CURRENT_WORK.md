...
The [Caliptra consequent-repetition candidate](session_logs/2026-09-23_caliptra_param_consequent_repeat_focus.json) records paired-edition executable assertion checks and a pinned formal-file compile with no dropped repeat properties; the [baseline](session_logs/2026-09-23_caliptra_param_consequent_repeat_baseline.json) preserves the original failure. It passed its required local gates and is open as [PR330](https://github.com/dsellerbrock/iverilog-uvm/pull/330), stacked on [PR329](https://github.com/dsellerbrock/iverilog-uvm/pull/329); both await current-head CI. The standalone bind-target error and full Caliptra DV/formal qualification remain separate.
