...
The [Caliptra long-sequence register fix](session_logs/2026-09-23_caliptra_long_sequence_register_focus.json) at `4ff0581df` passes paired 999/1000/1001-step executable checks, the focused negative and dual-engine SVA suites, and `make check`. The clean pinned PM formal file no longer reports duplicate `b999`; this branch still lacks PR334's separate late-default-clock fix and has 16 clock diagnostics. Integration, CI, full DV, and formal proof remain open. PR332 and stacked PR333 have merged to `main` at `5dacd0dec` and `bdf461a87` after exact-head Ubuntu CI success.

The [Caliptra consequent-repetition evidence](session_logs/2026-09-23_caliptra_param_consequent_repeat_focus.json) records paired-edition executable assertion checks and a pinned formal-file compile with no dropped repeat properties; the [baseline](session_logs/2026-09-23_caliptra_param_consequent_repeat_baseline.json) preserves the original failure. [PR330](https://github.com/dsellerbrock/iverilog-uvm/pull/330) merged at `d2820aac0` after [PR329](https://github.com/dsellerbrock/iverilog-uvm/pull/329) merged at `0725fd8eb`. The standalone bind-target error and full Caliptra DV/formal qualification remain separate.
