...
The current branch `agent/commercial-unsafe-20260923` is at source commit `9111127532c71fc4eeaf1bab68685ade3c61296f`. Its opt-in `-gcommercial-unsafe` mode has passing focused checks in [the revision-scoped record](session_logs/2026-09-23_commercial_unsafe_flag.json); no full suite ran on this flag head. The OpenTitan SPI compile clears the strict type errors with the flag but still reports unsupported nonblocking property assignment codegen errors; OpenTitan DV has not passed.
