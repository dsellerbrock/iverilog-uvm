...
| Iteration | elaborate.cc:22119 | **Re-triaged again (2026-09-11).** Still no reproducer reaching this exact branch (fourth attempt: too-many-index-variables `foreach(assoc[k,j])` over a 1D associative array of structs, `evidence/campaign-20260908/other8-audited/iter2_toomany_idx.sv`, added to the earlier three nested-container shapes). **But that attempt surfaced a DIFFERENT, unexplored, silent-ish fallback nearby**: the same construct compiles clean (exit 0) and then at runtime prints `function $low()/$high() argument object code is 615; using 0/-1 fallback` and the loop body never executes -- wrong result with only a runtime warning, not investigated further this pass (time-boxed; not the row this entry originally tracked). | Re-open elaborate.cc:22119 if a nested-container shape reaching it is found. Separately, a NEW candidate: trace the `$low()`/`$high()` "object code 615" runtime fallback triggered by too-many-index-variable `foreach` over a 1D associative array -- not yet root-caused. |
