...
### 2026-09-21 current VPI force-callback debt reproduced

Installed `09316da3d` reproduces the earlier callback debt in both 2017 and 2023 modes. Legal whole-variable force/release registrations fire, but their callback `obj` is type `vpiReg`, rather than the statement handle required by 38.36.1. A `cbForce` registration on the legacy `vpiRegBit` handle also succeeds despite the prohibition. The bit-select `cbRelease` rejection was not separately exercised. `make_force_release` already rejects newer packed variable-bit views but accepts legacy `vpiRegBit` in its shared bit-parent branch. Private sources, commands, tool hashes and logs are under `evidence/review-20260920/next-vpi-force-callback-assessment/`. This is reproduced next-batch debt, not an authorized implementation change or evidence of corrected behavior.

### 2026-09-21 vendor-compatibility investigation reminder

User requested investigation of documented VCS quirks/extensions when they may explain OpenTitan or Caliptra blockers. Candidate questions are strict-versus-glob legacy regex handling in OpenTitan and the scheduling-sensitive Caliptra rejection-sampling boundary. Current evidence establishes neither VCS behavior nor a required workaround. Before attributing such incompatibilities solely to application defects, check the pinned workload’s supported simulator/version/options and use documented behavior or a minimal differential run. Keep vendor compatibility separate from IEEE/UVM semantics; do not silently change standard behavior, modify application sources, or count a vendor-specific accommodation as standards qualification. Implementation remains subject to coordinator selection in ACTIVE_WORK.

### 2026-09-21 SPI Host failures after member-array constraint parser repair

Active blocker: OT-SPI-HOST-STRUCT-MEMBER-ARRAY-CONSTRAINT. Installed uncommitted candidate (tool hash and exact command in `evidence/review-20260920/spi-host-build/application-2/result.json`) with pinned UVM1.2 and unmodified Earlgrey-PROD-M6 passes the original `spi_host_base_vseq.sv:52` parser boundary, then exits139. Earlier diagnostics report queue/dynamic-array output element-type equivalence at `spi_host_driver.sv:156,256` and unsupported compound class-property event combination at line48. The crash follows those diagnostics; its root cause is not established. Do not count this as SPI Host compilation or DV success, or attribute the crash to the constraint patch without a reducer. Triage status: recorded, bounded assessment needed; no additional implementation selected. Raw compiler output is preserved in `evidence/review-20260920/spi-host-build/application-2/compile.log`.

### 2026-09-21 correction: OpenTitan regex failure includes a harness configuration gap

The earlier assertion that pinned OpenTitan/UVM regex configuration matched upstream is superseded by `evidence/review-20260920/opentitan-priority-spi-host-next-regex/ASSESSMENT.md`. Earlgrey-PROD-M6 `hw/dv/tools/dvsim/common_sim_cfg.hjson:65` supplies `+define+UVM_REGEX_NO_DPI`; matrix discovery records the option but the compiler command omits it. Pinned UVM1.2 uses this macro to select its SystemVerilog glob matcher. An unchanged direct `*_shadowed` match/nonmatch test fails through strict DPI regex without the define and passes with the upstream define while ordinary `-uvm` and other DPI remain enabled. Earlier runtime trace results remain valid for their recorded command, but do not prove an upstream-invalid pattern or VCS-specific behavior. Selected harness correction: OT-UVM-REGEX-CONFIG-PROPAGATION in ACTIVE_WORK. Full GPIO/pwrmgr DV outcomes remain unproven until rerun.

### 2026-09-21 GPIO exact distribution blocker after configuration correction

The corrected pinned GPIO run reaches normal UVM completion but remains unqualified. `evidence/review-20260920/opentitan-gpio-dist-next/ASSESSMENT.md` traces the runtime weighted-soft warning to inherited `cip_base_vseq.sv:66-73` reset-delay `:/` ranges with thousands to millions of members. A 301-member `:/1` versus 211-member `:/3` reducer selects only the heavier item in64draws; the sub-cap control samples both. This is probability-semantics debt, not a warning that can be waived or cured by increasing the enumeration cap. Exact range sampling must preserve item-versus-per-value weights, overlap, feasible support, solve ordering and RNG ownership. Read-only assessment continues; no new implementation is selected while PR311 qualification is under repair.

### 2026-09-21 power-manager descendant VIF event context

Read-only assessment after the upstream regex configuration repair identifies false null-at-arm errors in `pwrmgr_scoreboard.reset_coverage_collector` descendant forks. The enclosing automatic receiver is nonnull, but its publication precedes child event-context allocation; the child proxy initializes from a null shared default instead of the matching ancestor activation. Paired 2017/2023 reducers cover detached and retained parents, distinct concurrent roots, rebinding, nested descendants and context reuse; direct task waits pass and genuine-null controls retain their fatal diagnostic. Evidence and clause references: `evidence/review-20260920/opentitan-pwrmgr-null-vif-next/LIFETIME_MATRIX.md`. Candidate repair reuses the existing automatic object slot through an explicit-context accessor; no implementation is selected while PR312 qualification remains active.
...
### 2026-09-21 event-recipe null-method diagnostic consistency

Active blocker: OT-WHOLE-FUNCTION-CLASS-EVENT. Build4 paired 2017/2023 private probes show null receiver methods returning object, real, string, or void can execute inside an integral event recipe without a null-call diagnostic when their bodies do not read instance properties. The new receiver guard currently covers integral-return calls; actual null-property reads are separately diagnosed. Relevant symbols: `of_CALLF_OBJ/REAL/STR/VOID` and their virtual variants versus `event_expr_method_receiver_valid_` in `vvp/vthread.cc`. Evidence: `evidence/review-20260920/whole-function-event-next/final-review-null-return/` (eight edition-paired observations).

IEEE1800-2017/2023 §8.4 declares null access to non-static members illegal, its result indeterminate, and an error optional; §8.10 identifies properties and methods as members. These invalid-source probes establish inconsistent diagnostic coverage, not a valid-source conformance failure or a required semantic result. The raw probe labels saying BUG refer to the proposed diagnostic policy, not a normative requirement. Triage: record-only robustness debt, not selected implementation. Current null-access qualification covers only the tested integral-return and actual property-read cases; no all-return-type diagnostic claim.
...
- Triage: reproduced upstream-compatibility regression, Linux only. A fix must keep plusarg/option interleaving for dvsim while stopping option parsing at the input file for extended arguments.
