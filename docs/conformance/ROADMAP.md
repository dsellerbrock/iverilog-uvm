...
| ID | Item | Nat | Status | Blocked-by | Done when |
|----|------|-----|--------|-----------|-----------|
| M10-1 | Multidimensional open arrays (`svGetArrElemPtr2/3`) | F | **PARTIAL** | — | 2-D/3-D element access works. **Fixed two silently-wrong bounds accessors (R7):** `svIncrement` returned a hardcoded `-1`, but H.10.2 defines it as `+1` when `left <= right`, and every array that can be marshaled today is a 0-based ascending dynamic array — so it was wrong for all of them, and a C model stepping an index by it walked out of the array immediately. `svSizeOfArray` computed `length * elem_bytes`, right for 1-D but **0 for multidim** (a nesting's outer array is non-contiguous, so its `elem_bytes` is 0), so a C model sizing a buffer from it got zero. Both now derive from the real geometry: increment from the bounds, total size from the product of all dimensions times the LEAF element size. `svLow`'s hardcoded 0 is left, and is correct today only incidentally (dynamic arrays are 0-based) — noted in the source. **Remaining:** marshaling a FIXED-size unpacked array (`int a[3:10]`) into dynamic-array storage, which is what would make non-zero and descending declared ranges observable and needs per-dimension bounds carried through `vvp_dpi_open_array_t`. That is now a loud sorry rather than the empty array it silently produced — see M10-1c. test m10_dpi_open_array_bounds_test |
...
| M10-1c | Whole fixed-size unpacked array used as an object produced an empty array | C | **DONE** (loud) | — | `tgt-vvp`'s `IVL_EX_ARRAY` object path emitted element 0 as the object, so all three shapes that reach it were silently wrong: `d = a;` (fixed -> dynamic, legal per 7.6) and a fixed array passed to a DPI open-array formal (legal per 35.5.6.1) both yielded an **empty** dynamic array — a C model saw `size 0` — and `h = arr;` (a type error) compiled as `arr[0]`. Now a sorry that states marshaling a fixed-size array into dynamic-array storage is unimplemented, and separately notes that the single-handle form is a type error. **Deliberately not phrased as a type error overall:** two of the three shapes are legal SystemVerilog and it is this implementation that is missing. Implementing the marshaling closes this and the rest of M10-1 together. negative object_array_to_handle, object_array_as_handle_arg |
...
1. **P0 silent wrong results — these preempt everything** (rule gate 1):
   ...
   - ~~**M10-1** object-array element load ignores its index~~ —
     **misdiagnosed**; indexing was already correct (M10-1b retired). The
     real defect on that path was a whole fixed array used as an object
     yielding an EMPTY array, now a loud sorry (M10-1c). The two silently
     wrong bounds accessors are fixed. Nothing here is silent any more.
...
| R7 | Marshaling a FIXED-size unpacked array into dynamic-array storage is unimplemented, so a declared non-zero or descending range (`int a[3:10]`, `int a[10:3]`) cannot reach a DPI open array or a dynamic array at all. `svLow` still returns a hardcoded 0, correct only because everything that does marshal is 0-based. | M10-1 | **yes** — sorry from tgt-vvp | Marshal the fixed array into contiguous storage with copy-back, and carry per-dimension low/high/increment through `vvp_dpi_open_array_t` so `svLow`/`svLeft`/`svRight` report the declared range |
...
**No residual in this list is silently wrong any more.** R7 was the last one,
and closing it took two passes: the two bounds accessors that were silently
wrong (`svIncrement`, `svSizeOfArray`) are now fixed, and what remains of it
— fixed-array marshaling — is a loud sorry. Every entry above is therefore
either loud, deliberate, or cosmetic, which is the bar the loud-sorry rule
sets. That does not make the list finished; it makes it honest.
