...
| M10-1c | Whole fixed-size unpacked array used as an object produced an empty array | C | **DONE** | — | Fixed by the marshaling in M10-1: `d = a` and a fixed array passed to a DPI open-array formal both work now, with the declared range reported. The remaining shape is a fixed array of **class handles** (`C arr[4]; C q[]; q = arr;`), which is legal and would marshal — but codegen cannot see the target type, so the same expression also covers `h = arr`, a type error. Marshaling both silently accepted the illegal one (it re-broke tests/negative/object_array_to_handle when tried), so handle-element arrays stay a loud sorry that says exactly why. DPI open arrays cannot have class-handle elements at all, so nothing is lost; closing it needs the type error caught at **elaboration**, where the target type is known. negative object_array_to_handle, object_array_as_handle_arg |
...
1. **P0 silent wrong results — these preempt everything** (rule gate 1):
   ...
   - ~~**M10-1** object-array element load ignores its index~~ —
     **misdiagnosed** (M10-1b retired; indexing was already correct). The
     real defect was a whole fixed array used as an object yielding an
     EMPTY array — now **fixed**, with the declared range reported and the
     declared index translated. M10-1 is DONE; M10 has no open items but
     the handle-element shape in M10-1c.
...
**No residual in this list is silently wrong any more, and R7 is now closed
outright** — it took three passes: the two hardcoded bounds accessors, then
