...
| R14 | An EXPLICIT clocking-event argument — `$past(e, n, gate, @(posedge clk))` — does not parse: the argument grammar takes expressions, and an event control is not one. | M9-12 | **yes** — syntax error | A grammar rule accepting an event control as the last argument of a sampled value function, then routing it to the sampler builder the default-clocking path already uses (`pform_make_sampled_history_process_` takes the event list). The lowering is done; only the parse is missing |
