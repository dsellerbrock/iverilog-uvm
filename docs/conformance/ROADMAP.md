...
| M1C-4 | **SILENT**: an unpacked-array interface member through a VIRTUAL interface reads `x` and drops writes | C | **OPEN** | — | `vif.arr[i]` reads `x` and `vif.arr[i] = v` is silently discarded, with no diagnostic and a clean exit; the same member accessed directly (`sif.arr[i]`) is correct, and every scalar member shape through the same virtual interface is correct. The write is the dangerous half — a driver driving an interface array through a virtual interface, the ordinary UVM shape, silently drives nothing. The lowering already emits an indexed property access (`%prop/v/i` / `%store/prop/v/i`), so the index reaches the runtime; what it indexes is the question. An interface is mirrored into a `netclass_t` in elab_type.cc (`iface_type->set_property(...)` over `mod->wires`) with the property's array storage coming from the elaborated member TYPE, so the first check is whether `elaborate_sig_type` yields the unpacked-array type or just the element type — a single-slot property explains both symptoms. **This is the same shape as M1C-3** (an index with nowhere to go), one mirror over. Repro: `repros/vif_unpacked_array_member_silent.sv`. Done when the repro prints the wanted values and a regression pins read/write, constant and variable index, and a packed-array member sibling |
