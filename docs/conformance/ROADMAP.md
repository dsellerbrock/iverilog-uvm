...
| M3B-10 | `std::randomize(vars) with {...}` does not reach the solver | F | OPEN (loud) | — | The scope form is lowered in the parser by heuristic: a range fast path (`$urandom_range` from collected `<`/`>`/`inside` bounds), an enum pick, else a bounded retry loop — so simple constraints hold and tight ones can fail to converge. In EXPRESSION context (`if (std::randomize(a,b) with {...})`) the with-clause is not enforced at all and the variables get unconstrained values; that path warns once, loudly. Without a with-clause `std::randomize(vars)` is correct. **Terms:** route the scope form through the same Z3 path class `randomize()` uses — the variables are not class properties, so it needs solver variables bound to plain signals rather than `p:IDX:WID` slots |
...
| ID | Item | Nat | Status | Blocked-by | Done when |
|----|------|-----|--------|-----------|-----------|
| M10-6 | An OUTPUT open-array formal arrived EMPTY and the empty formal was copied back OVER the actual | C | **DONE** | — | For an ordinary SV subroutine an output argument is write-only (13.5.2) and the formal starts uninitialized, so the task-call lowering skipped the copy-in on direction alone. A DPI open array is not that: 35.5.6.1 and H.10.2 make the ACTUAL determine the array's shape, and the C side reads that shape back through `svSize`/`svLow`/`svHigh` before it writes an element — only the element VALUES are outputs. So `import "DPI-C" function void c_fill(output int a[]); dyn = new[4]; c_fill(dyn);` handed C an unallocated formal: `svSize` returned **0** and `svLow`/`svHigh` the degenerate pair **-1/0**, which makes the natural `for (i = svLow; i <= svHigh; i++)` loop run TWICE out of bounds, `svGetArrElemPtr1` returned NULL for every index, and on return the empty formal was copied back over the actual — leaving `dyn.size() == 0`, the caller's array **destroyed**, with no diagnostic and a clean exit. Fixed by copying in an open-array formal whatever its direction, restricted to DPI imports (so ordinary SV output arguments keep 13.5.2) and to dynamic-array/queue formals (the only ones whose shape the callee must read). **`inout` was always correct** — inout ports were already copied in — which is the control in the test, along with `input`. The defect is specific to a VOID import called as a STATEMENT: a value-returning import used in an expression takes the function path, which always copied every argument in. tests/m10_dpi_output_open_array_test (mask=15 and a destroyed actual against a reverted build) |
...
| R15 | A sampled value read from a context NOT aligned with its clocking event — `$past(e, 1, , @(posedge other))` evaluated at a moment that is not a tick of `other` — returns the sample at the most recent tick rather than the one before it, i.e. it is one tick young. Aligned reads (the shape 16.9.3 defines the value for, and every shape the tests pin) are exact. | M9-12b | no | The history shifts under NBA, which makes an aligned read exact and an unaligned read one young; only a Preponed-region shift is exact for both, and a process cannot detect a clock edge in the Preponed region of the slot that edge occurs in. Closing it needs the sampler moved off the process model onto the runtime's existing preponed value history (`%load/preponed`, added for M6B-4), which is a different lowering rather than a tweak |
| R16 | `$cast` into an element of a container that is NOT a direct class property — a module- or block-scope `Base q[$]`, or one reached through a nested receiver (`p.inn.q[i]`) — returns 1 and writes nothing; into a local FIXED array element (`larr[i]`) it aborts the runtime. Those destinations parse as a SELECT over a container read rather than as an indexed property, so they miss the property lowering and fall through to the generic VPI path, which cannot put an object value into them. | M1C-6 | no (the local fixed-array case aborts loudly; the container cases are silent) | The same store sequences the assignment path already emits, dispatched off the SELECT's sub-expression type: `%store/obja` for a fixed array, `%store/qdar/obj` for a queue, `%store/dar/obj` for a dynamic array and `%aa/store/sig/obj/*` for an associative array — four shapes, each with its own index register and operand form, so each wants its own verification rather than one shared branch |
