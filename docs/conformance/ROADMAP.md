...
| M4C-8 | **NBA through a class handle or virtual interface executes immediately** (blocking semantics): `c.v <= x` / `vif.sig <= x` visible before the NBA region — silent event-region violation (IEEE 1800-2017 4.9.3, 10.4.2); UVM drivers rely on this | C | OPEN (confirmed, next) | — | repro r1.sv (scratchpad/m4c7); needs NBA-scheduled property stores |
| M4C-9 | Member write into an assoc element of PACKED struct type aborts iverilog (elab_lval.cc:2479 `name_base.index.size()+1 == reg->packed_dimensions()`) | R | OPEN (confirmed) | — | repro r3.sv (scratchpad/m4c7) |
