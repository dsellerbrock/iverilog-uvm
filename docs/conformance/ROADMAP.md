...
| R14 | A sampled value function with no INFERABLE clock — written in an `initial` block, a task/function body, or a continuous assignment — falls back to the unsampled value. So does an explicit clocking-event argument (`$past(e, n, gate, @(posedge clk))`), and the `default clocking` fallback 16.14.6 offers when no event control encloses the call. | M9-12 | **yes** — per-call warning naming 16.9.3 and saying exactly what the fallback answers | Bind these to a SEPARATE sampler process on the named (or default) clock rather than splicing into the reader's block. That process cannot use blocking assignments — it would race the readers on the same edge — so the history has to shift under NBA and `$rose`/`$fell` must read their current sample live rather than from a register. Straightforward, but it is a second lowering shape and wants its own verification of the ordering claim |
