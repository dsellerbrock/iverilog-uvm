...
|---|----------|------|-------|------------------------|
| R2 | Assertion **evaluation** runs in the Observed region. Action blocks still run there too, rather than in Reactive, and per-attempt start/step/end events are not placed individually. | M6B-4 | n/a — not wrong, one region early | **Deferring actions to Reactive was implemented and reverted: it drops verdicts.** A `final`-block action — which is how a strong sequence reports an unfulfilled obligation at end of simulation — suspends into Reactive and never resumes, and the last tick's action is lost to `$finish`; `strong_weak_nfa_only` lost its `ERROR` line and `seq_or_and_nfa_only` lost `s1 FAIL at 35`. Losing a failure report is worse than reporting it one region early, so this needs `$finish` to drain pending Reactive events first, or an action path that survives end-of-simulation — not more plumbing |
