...
|---|----------|------|-------|------------------------|
| R2 | Assertion **evaluation** now runs in the Observed region; what is left is the finer per-attempt start/step/end placement and running action blocks in Reactive rather than inline. | M6B-4 | n/a — not wrong, absent | Evaluation moved (see below). The remainder needs each attempt's start/step/end events placed individually, and the pass/fail action re-scheduled into the Reactive region — the runtime has that region (`SEQ_REACTIVE`, `vthread_is_reactive`), so it is plumbing rather than new machinery |
...
| R11 | Two operand shapes still read live, both because `%load/preponed` cannot reach them at the far end: an **unpacked-array word** (the opcode takes a signal and ignores the word index, so sampling one would read word 0) and a **real** (no real-valued form of the opcode exists). | M6B-4 / R1 | **yes** — per-operand warning at elaboration naming the operand and the consequence | A word-indexed preponed load and a real-valued one: two new opcodes plus driven-value history on the corresponding functors. Runtime additions, not rewrite fixes. **Two of the original four shapes are closed:** hierarchical names now sample (the history enable is a system task taking the signal, so a hierarchical reference from the checker's own initial block reaches the other scope perfectly well — it had simply been excluded), and the copier now handles real and string literals, which had made `r > 1.0` fall back to a live read for the WHOLE guard. Only a package-qualified name still falls in the copier, and it warns |
