...
| ID | Item | Nat | Status | Blocked-by | Done when |
|----|------|-----|--------|-----------|-----------|
| M1C-1 | ICE: `foreach` over a runtime-sized array in a `$unit`-scope class constructor | R | OPEN | — | Compiler ABORTS on legal input: `assert: elaborate.cc:11284: failed assertion idx_sig`. Minimized boundary (repro `repros/foreach_in_unit_class_constructor_ice.sv`): only the CONSTRUCTOR (the same body in `function void init()` is correct), only a dynamic array or queue (a fixed-size array property takes the compile-time-bounds path and is fine), only at `$unit` scope (the same class inside a module works), and it applies to method locals as well as properties. Evidence gathered: at the failure point `$unit.H.new.$ivl_foreach0` exists as a child of the constructor scope but holds no signal `i` — not there, not in the constructor scope, the class scope or `$unit`. `PFunction::elaborate_sig` runs once for the constructor (elab_stage 1→2), its walk reaches the block (`PBlock::elaborate_sig` does not report a missing child scope), so `elaborate_sig_wires_` ran against an EMPTY wire list. **The pform side is correct** — instrumenting `pform_make_foreach_declarations` shows the loop variable declared into the pushed `PBlock` in both the failing constructor and the working control ("kind=PBlock, scope now has 1 wires"), so the loss is on the elaboration side. It is also **not** two NetScope trees (the same scope object appears in `netclass_t::elaborate_sig` and at the failing `elaborate_runtime_array_`, at elab_stage 1 with a statement attached), and **not** a divergent signature walk: tracing `PBlock::elaborate_sig` shows the failing constructor and the working control both descending into `$ivl_foreach0` with `wires=1`. Everything up to and including the call to `elaborate_sig_wires_` is identical, and only one of them ends up with a signal. **Next step: inside `elaborate_sig_wires_` / `PWire::elaborate_sig` for that one wire** — the index's `foreach_index_type_t` resolves through `resolve_class_handle_placeholder_type_weak_`, which returns 0 for a non-class index type; whether the caller treats 0 as "skip this wire" is the first thing to check. Done when the repro compiles and runs, with a permanent regression test covering property + local × darray + queue × `$unit` + module |
