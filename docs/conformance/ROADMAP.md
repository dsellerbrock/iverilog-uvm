...
| M4C-2 | Indexed struct-member READ through a class-property chain prints a NON-FATAL `sorry` (iverilog exits 0), yields x, and if-conditions const-fold to the wrong branch — silently deleting user checks | C | OPEN (next) | — | audit findings; site: elab_expr.cc chain walker "Indexed struct member access not yet supported" |
| M4C-3 | Bit/part-select READ of unpacked-struct vector members returns x/0 (plain variable, and inside assoc/queue/darray elements); `[b -: w]` on container struct members evaluated as `[b +: w]`; `$display` arg replaced by literal " " | C | OPEN | — | audit findings (struct-read, container-read lenses) |
| M4C-4 | OOB READ of a queue-of-struct element GROWS the queue (regression from M4B-9's queue `declared_type`: `of_LOAD_DAR_OBJ` materializes+`set_word` past size) | C | OPEN (regression, next) | — | audit finding; guard materialization to in-bounds |
| M4C-5 | Reading a member of a nonexistent assoc key returns 0 instead of x for 4-state members; part-select read of q[$] element segfaults iverilog at compile time; nested unpacked-array property chain read (`o.inr.arr[i]`) returns x | C | OPEN | — | audit findings (container-read, symmetry lenses) |
| M4C-6 | Multi-dim packed WRITE residuals from md-adversary: ascending-range element read picks wrong element; OOB/x-index writes corrupt element 0 or spill into neighbors; OOB constant write to plain multi-dim var asserts at compile time | C | OPEN | — | audit findings (md-adversary lens) |
| M4C-7 | 12 unverified findings (symmetry/op-adversary verifier pass was cut by usage limits) incl. NBA-through-handle executes immediately, compound-op crashes on unpacked-array element parts — re-verify before implementation | A | OPEN (re-verify) | — | scratchpad/audit summary.json 'rejected' list |
