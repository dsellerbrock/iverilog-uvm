...
| M6B-3 | Scheduling for time-consuming DPI imports | F | **VERIFIED WORKING** | M6-CALLF | An imported `context task` consumes simulation time via the coroutine path, and it participates in normal scheduling: `fork`/`join_any` + `disable fork` correctly abandons a blocked DPI import and its exported task never completes (probe m6b3_kill). **Terms:** pin the probe as a regression test, then close. The *concurrency* defect found alongside it belongs to M10-4, not here |
| M6B-4 | Assertion attempt-lifecycle scheduling | F | OPEN — **P0 silent wrong result** | ARCH M9-NFA | **Concurrent assertions sample Active-region values, not Preponed** (IEEE 1800-2017 16.5.1). Deterministic repro (probe m6b4_det): one thread does `a = 1; clk = 1;` in the same time slot; `assert property (@(posedge clk) a \|-> b)` must sample the preponed `a` (=0) and be vacuous, but Icarus sees `a`==1 and reports a failure. Signals written by **NBA** happen to behave correctly (probe m6b4_nba) only because NBA updates land after edge detection — there is no Preponed implementation, just accidental ordering. **Terms:** maintain preponed shadow copies of every signal referenced by an assertion expression, refreshed in a pre-Active scheduler pass, and make the NFA checker read the shadows instead of live values at clock-edge detection. Per-attempt start/step/end region placement follows once sampling is correct |
...
| M10-4 | Time-consuming imported tasks | F | OPEN — **two P0 silent wrong results** | **M6-CALLF** | Single static-lifetime exported task via an imported `context task` works, and two *different* exported tasks running concurrently work (probes m10_4_slowtask, m10_4e_diff). Two silent defects, no diagnostic in either case: **(a) `automatic`-lifetime exports are broken outright** — any exported `function`/`task` with automatic lifetime receives **`x` for every argument**, even on a single non-concurrent call, so a value-returning export returns garbage and `#(d)` degenerates to zero delay (probe m10_4f_auto1). **(b) the same exported task invoked concurrently aliases its arguments** — every live invocation reads the last-marshaled values (probe m10_4c_trace: `sv_wait(3,1)` ran as `id=0 d=5`). **Root cause (both):** `compile_export_dpi` (vvp/compile.cc:1250-1263) resolves `arg_nets` once at link time to the **static prototype nets**, and `dpi_export_run_` (vvp/vthread.cc:10684-10705) marshals into them; an automatic scope's body reads its per-invocation `%alloc` context and never sees the static net, while concurrent static invocations share the single net. **Terms:** minimum bar per the loud-sorry rule — in tgt-vvp's export emission (vvp_scope.c, alongside the existing unsupported-arg-type drop) reject `ivl_scope_is_auto()` exports with a sorry so the C symbol fails to link, and in `dpi_export_run_` track a live-invocation flag per export record and refuse a second overlapping invocation loudly. Full fix — marshal into the child's allocated context *after* its `%alloc` runs, giving per-invocation argument storage — clears both |
...
M10-5) and four silent defects were hiding under "OPEN" rows that read as
merely unimplemented (M3B-5, M6B-4, M10-4 ×2, plus M13-5/6/7 as
accepted-noops). The lesson that drove this sweep holds: **probe
boundaries, not headline features** — every headline worked.

1. **P0 silent wrong results — these preempt everything** (rule gate 1):
   - **M10-4(a)** `automatic`-lifetime DPI exports read `x` for every
     argument, even on a single call. Cheapest correct step is the loud
     drop in tgt-vvp; the real fix is per-invocation arg storage.
   - **M10-4(b)** concurrent invocations of one exported task alias
     their arguments. Same root cause, same fix.
   - **M6B-4** concurrent assertions sample **Active**, not **Preponed**,
     values — wrong assertion results on ordinary legal SV. Largest of
     the four, and it touches the NFA engine's value sourcing.
