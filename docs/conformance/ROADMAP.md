...
| M4B-8 | Partial write to a **virtual-interface struct field** (`vif.pkt.a = …`, `vif.pkt.b[3:0] = …`) silently miscompiles (whole clobber / dropped) | C | OPEN (tracked) | — | write-path-audit finding; the VIF member l-value path does not apply the packed-struct-field / part-select RMW that the class/struct paths now do |
