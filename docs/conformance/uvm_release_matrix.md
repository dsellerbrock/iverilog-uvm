...
2026-09-11, native ARM64, validated source `9229c24e7` (L29 procedural-block recovery).
The release sweep uses classifier `1654dc4c9` (U12); all required L29 local
...
| --- | --- | --- |
| 1.0p1 | SMOKE_PASS | All smoke checks passed through time1 after L28; zero UVM warnings/errors/fatals. Existing ignored-constraint and unsupported-cast compile warnings remain |
| 1.1a | SMOKE_PASS | All smoke checks passed through time1 after U11; zero UVM warnings/errors/fatals |
| 1.1b | COMPILE_FAIL | Undefined `uvm_record_attribute` macro, syntax errors (normal rejection, no parser assertion) in `uvm_tlm2_generic_payload.svh` |
| 1.1c | COMPILE_FAIL | Undefined `uvm_record_attribute` macro, syntax errors (normal rejection, no parser assertion) in `uvm_tlm2_generic_payload.svh` |
...
All 15 sources were acquired; 13 compiled and passed the bounded runtime smoke,
and 2 failed compilation. Compile-time limitations remain visible; these are
not warning-free compilation or full feature-qualification claims. These are observed compatibility gaps, not waived
...
`third_party/uvm-releases/results-bwek89ja/results.json`, with per-release
...
L28 final validation: 77cce610b; counter and callback reds both editions PASSED/no diagnostics; focuslegacy4/0,JSON4/0,static/identityneighbors37legacy/25JSON,L27neighbors2/0each,makecheck and independent design/code/evidence review; legacy4955total4950pass0fail2NI3EF,VPI105/0,negative149/0,runtime15/15,copyout6/6,exports66,fullJSON1847/0,realDPIUVM355/0/0,NFA58/58,frontend91254exit0 allscenarios;installation restored/five frozen hashes match. Complete15release results-bwek89ja13SMOKE_PASS2COMPILE_FAIL;only1.0p1 statusimproves,all15source/archivehashes unchanged. Original1.0p1 cleanruntime/time1 with zeroUVMwarnings/errors/fatals;existingcompileconstraint/castlimitations remain,notfullUVMqualification.
