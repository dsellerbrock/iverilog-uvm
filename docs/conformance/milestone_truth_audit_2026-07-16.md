...
| M12 VPI SV object model | CLOSED | **SUBSET COMPLETE** | Classes/members/containers/interfaces/modports/packages/covergroup-handles/value-change-callbacks are modeled, and now **concurrent assertions have VPI identity** (`vpi_iterate(vpiAssertion, 0)` and scope-scoped, with vpiName/vpiFile/vpiLineNo/vpiScope — this session, via runtime registration of the synthesized checkers). Still open: assertion **callbacks** (cbAssertionStart/Success/Failure) and force/release on bit-selects + cbForce/cbRelease. **M12B (narrowed).** |
...
engine. Assertion VPI **enumeration** (M12B) is now done; the remaining
bounded items are assertion **callbacks** (cbAssertion*), VPI
force/release on bit-selects, and pieces of **M1B** (semantic-IR
