...
| M12 VPI SV object model | CLOSED | **SUBSET COMPLETE** | Classes/members/containers/interfaces/modports/packages/covergroup-handles/value-change-callbacks are modeled; **concurrent assertions have VPI identity** (`vpi_iterate(vpiAssertion, 0)` and scope-scoped, with vpiName/vpiFile/vpiLineNo/vpiScope, via runtime registration of the synthesized checkers) and now a live **callback surface** — `vpi_register_assertion_cb` with **cbAssertionSuccess/cbAssertionFailure** fires on attempt resolution, keyed by (scope, idx) so multiply-instantiated checkers stay distinct (M12B-cb, this session). Still open: the remaining cbAssertion* reasons (Start/Step/disable) + populated `s_vpi_attempt_info.detail`, and force/release on bit-selects + cbForce/cbRelease. **M12B (narrowed).** |
...
engine. Assertion VPI **enumeration** (M12B) and **success/failure
callbacks** (M12B-cb: cbAssertionSuccess/cbAssertionFailure via
`vpi_register_assertion_cb`) are now done; the remaining bounded items are
the other cbAssertion* reasons (Start/Step/disable) with a populated
`s_vpi_attempt_info.detail`, VPI force/release on bit-selects, and pieces
of **M1B** (semantic-IR remediation). None is a silent miscompile.
