...
| OpenTitan SPI Host | Add `-gcommercial-unsafe` **only** when testing the opt-in packed `bit`/`logic` queue conversion. It clears the recorded strict type errors. The [current selected-bit NBA candidate](../session_logs/2026-09-23_opentitan_nba_codegen_focus.json) also removes the five target errors, but its compile still drops four constraint-index references and one queue-pop expression; SPI DV is not qualified. The flag merged in [PR 326](https://github.com/dsellerbrock/iverilog-uvm/pull/326); [post-merge local gates passed](../session_logs/2026-09-23_pr326_postmerge_qualification.json). |
