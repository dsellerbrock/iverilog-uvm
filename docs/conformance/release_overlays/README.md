...
| OpenTitan ADC Control `adc_ctrl_sim` | No source overlay is selected. The [pinned target result](../session_logs/2026-09-23_opentitan_adc_macro_arg_focus.json) uses FuseSoC's generated source list, `-g2012 -uvm` with pinned UVM 1.2, and the target's UVM, DUT, and bind options. Metadata selects `adc_ctrl_base_test` / `adc_ctrl_smoke_vseq` with `+smoke_test=1`. The macro fix permits compilation; the later [guarded-distribution and fileset checkpoint](../session_logs/2026-09-23_opentitan_adc_guarded_dist_fileset_focus.json) shows the exact-dist failure is gone, but smoke still fails at time 0 on unsupported class-handle collection resizing. No ADC DV pass is claimed. |
