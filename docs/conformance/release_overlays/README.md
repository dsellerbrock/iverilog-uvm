...
pinned release sources below with the patch and run options selected for the
particular test. This shorthand does not mean every patch is applied to every
test, or that either full DV suite passes. Keep the pinned checkouts pristine;
apply overlays only to disposable source copies. Record unmodified-source
results separately.
...
| OpenTitan xbar `dv_report_catcher`: **conditional historical patch**, not part of the default xbar profile. The September 15 passing run used it to replace nonstandard `foreach` syntax; a later bounded xbar smoke passed with this source unmodified. | [dv_report_catcher_foreach.patch](opentitan/dv_report_catcher_foreach.patch) | [Original syntax and patched run](../session_logs/2026-09-15_opentitan_xbar_smoke_patched.md), [later release replay](../session_logs/2026-09-21_qualified_nine_release_retest.json). |
...
the patches to the pinned checkouts. Patch SHA-256 values, in table order, are
`dc4c9b4be456a842e34c41cb002972762395ac0952bcef153d3bca7b4e9af8ec`,
`dec3ee052af0122b4c90e86a47aa5421e1222f73608bddbe9bbb5cb979494a45`,
and `9213d50c02d9477a16a66daa744bb906b69e3634de7ad1955f5d879f4fcfbb3d`.
...
| OpenTitan SPI Host | Carry upstream's `-DUVM_REGEX_NO_DPI` from `common_sim_cfg.hjson` into the Icarus compile; it selects UVM's SystemVerilog glob matcher for patterns such as `*_shadowed`. Keep the normal real-DPI bundle for other UVM functions. Add `-gcommercial-unsafe` **only** when testing the opt-in packed `bit`/`logic` queue conversion. It clears the strict type errors. The [selected-bit NBA candidate](../session_logs/2026-09-23_opentitan_nba_codegen_focus.json) also removes five target errors, but four constraint-index references and one queue-pop expression are still dropped. With the regex option, the pinned smoke reaches a VIF event-relay runtime abort; SPI DV is not qualified. The [earlier regex configuration record](../session_logs/2026-09-21_opentitan_regex_configuration.json) explains the required option. |
