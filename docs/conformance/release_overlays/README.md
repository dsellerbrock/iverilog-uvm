...
| OpenTitan SPI Host | Add `-gcommercial-unsafe` **only** when testing the opt-in packed `bit`/`logic` queue conversion. It clears the recorded strict type errors, but [later codegen errors remain](../session_logs/2026-09-23_commercial_unsafe_flag.json); SPI DV is not passing. The flag merged in [PR 326](https://github.com/dsellerbrock/iverilog-uvm/pull/326); [post-merge local gates passed](../session_logs/2026-09-23_pr326_postmerge_qualification.json). |
