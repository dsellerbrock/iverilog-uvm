...
- **Closure:** The typed `pop_front`/`pop_back` result, single queue mutation, empty-queue boundary, invalid arity, and neighboring checks pass locally. [Boundary follow-up](session_logs/2026-09-23_queue_pop_assoc_boundary.json) rejects these queue methods on associative arrays but preserves an associative-array element whose type is a queue. Broad required gates and current-head CI remain; full SPI DV is separate and still blocked by four constraint-index warnings.
