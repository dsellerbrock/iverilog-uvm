...
- **State:** The local candidate proves a joint distribution's complete guard active or inactive under the hard constraints before drawing. Paired 2017/2023 constant, hard-pinned, nested-inactive, weighted, unresolved, and rollback reducers pass. The released `csrng_smoke_test` still fails at `dv_base_test.sv:41`: a later random guard remains unresolved, so the solver reports it explicitly without treating its distribution as inactive. Conditional hard distributions are legal under §§18.5.4/18.5.7 (2017) and §§18.5.3/18.5.6 (2023). Full conditional-fiber sampling is still required.
- **Evidence:** [Current candidate replay](../../evidence/opentitan-csrng-proven-guard-replay-20260923/result.json), [paired reducer/trace](../../evidence/opentitan-csrng-cfg-joint-dist-20260923/README.md), and [focused tests](session_logs/2026-09-23_joint_proven_guard_focus.json). The replay's exit-0 process contains `UVM_FATAL`, `UVM_ERROR`, and EndOfSimulation assertion failures and is not a DV pass.
