...
### OT-SRAM-INDEXED-DARRAY-SLICE — runtime-indexed dynamic-array slice values

- **State:** The reviewed private compiler passes paired IEEE 1800-2017/2023 read, write, overlap, mismatch, invalid-form, control, and resource-boundary tests (16/16). Full legacy has 6,556 ordinary passes of 6,561 total, zero failures, two not implemented, and three expected failures; full JSON passes 3,548/3,548; negatives 155/155; VPI 131/131; and real-DPI UVM 358/358 with zero failed/skipped. The exact pinned `otp_ctrl_sim` and `chip_sim` replays clear all four former `sram_scrambler_pkg.sv:278/280/332/334` errors, then fail on 22 and 60 classified hard diagnostics, respectively; chip also aborts on a separate `entropy_src_core.sv:3324` elaboration assertion. Neither enters DV runtime. The installed compiler remains unchanged during Caliptra's live sweep. See the [reviewed candidate evidence](../../evidence/opentitan-sram-indexed-darray-slice-20260924/README.md).
- **Requirement:** IEEE 1800-2017 §§7.4.6, 7.6 and 2023 §§7.4.5, 7.6 require a fixed-count unpacked slice at a variable base, equivalent element types, left-to-right assignment, invalid-index defaults/no-op writes, and no target mutation on a mismatched dynamic source size.
- **Boundary:** The candidate handles direct one-dimensional blocking reads/writes, integral bases up to 127 bits, and a complete RHS snapshot. Legal bases wider than 127 bits, slices wider than 65,536 elements, and nonblocking or timed indexed-slice assignments remain explicit unsupported cases. Property/nested and multidimensional forms are not claimed.
...
### OT-CSRNG-CFG-JOINT-DIST — published ordered conditional-fiber sampler

- **State:** Private source-tree Icarus implements bounded exact ordered conditional-fiber sampling in vvp/vvp_z3.cc. It proves guards and due weights across every feasible earlier-stage prefix before RNG, samples inactive subjects by the ordinary exact path, and rejects unproved coupled/randc/UNKNOWN/cap cases loudly. Existing guard focus passes 22/22, initial new paired cases 6/6, added independent-weight/randc/prefix-cap cases 6/6, registered legacy focus 34/34, and registered JSON focus 34/34. The final expanded broad legacy gate passed 6581 total with 6576 ordinary passes, zero failures, two not implemented, three expected failures, bundled VPI 131/131, negatives 155/155, and runtime invariants 15/15; private real-DPI UVM passed 358/358. Full JSON passed 3616/3616 with zero failures. Pinned CSRNG and OTP advance beyond cfg.randomize() but remain **0/1 DV each** at time zero on separate nested-VIF handoff and class-specialization identity blockers, with no pass marker or meaningful traffic. The solver patch is published in [PR #358](https://github.com/dsellerbrock/iverilog-uvm/pull/358); private binaries are not installed.
- **Requirement:** Conditional hard `dist` constraints are legal under §§18.5.4/18.5.7 (2017) and §§18.5.3/18.5.6 (2023). When an earlier `solve ... before` stage resolves a guard, prove activation for every feasible prefix before any RNG draw; retain exact active weights, ordinary exact inactive-subject sampling, and rollback. Same-stage or unproved guards, UNKNOWN, unsupported weights/caps, and randc interactions must remain loud rather than being treated as inactive or sampled with bias.
- **Scope:** The published implementation changes only `vvp/vvp_z3.cc` and dedicated paired 2017/2023 constraint tests/manifests. The coordinator owns private build, broad gates, pinned CSRNG/OTP replays using named disposable overlays, evidence, review, and PR. Installed Caliptra tools and pinned source checkouts remain unchanged.
- **Evidence:** [Existing candidate and pinned CSRNG replay](../../evidence/opentitan-csrng-proven-guard-replay-20260923/result.json), [paired CSRNG reducer/trace](../../evidence/opentitan-csrng-cfg-joint-dist-20260923/README.md), [focused proven-guard tests](session_logs/2026-09-23_joint_proven_guard_focus.json), [pinned OTP overlay replay with nested cfg reducers](../../evidence/opentitan-otp-backdoor-path-overlay-20260924/README.md), and [paired coupled-child RED](../../evidence/opentitan-joint-ordered-guard-red-20260924/README.md). The OTP compile also has 46 nonfatal `sorry` diagnostics and an ignored constraint item; those remain separate DV qualification gaps. [Private focus and released replays](../../evidence/opentitan-joint-ordered-guard-candidate-20260924/README.md). [Registered paired review-branch cases](../../ivtest/regress-joint-proven-guard-legacy.list).

### OT-CSRNG-NESTED-VIF-HANDOFF — released genbits VIF value lost

- **State:** The scoped `elab_expr.cc` and `tgt-vvp/eval_object.c` fix selects the named child of the class-held parent VIF at runtime and preserves that physical child's instance identity. Paired strict 2017/2023 legal, missing, wrong-type, hidden-modport, null, rebind, and parameterized-layout focus passes 14/14 in both harnesses; neighboring VIF cases pass 184/184 each. A declared child with no physical parent now compiles and produces a null-receiver runtime fatal. The final private `ivl` is `a2470636e85fced3aacc6df62f790e32615e339f8dca012f18191d60eaa708bd`. Final broad legacy passes 6,601 total with zero failures, full JSON 3,640/3,640, and real-DPI UVM 358/358.
- **Released DV:** Pinned CSRNG with its named disposable BlkLen overlay and explicit `-gcommercial-unsafe` compiles with the final private tool, but remains **0/1 nonstandard compatibility DV**. It advances past the former 0 ps null nested-VIF failure to 1,000 ps, then fails an unknown CSR backdoor field check, reports `TEST FAILED CHECKS`, a TL-UL assertion, and exit 1. No meaningful CSRNG command traffic or scoreboard comparison occurs. The exact unknown-field cause is unresolved. The compiler still skips a `mon_cb` monitor event, assumes a monitor condition false, and ignores an `int_state_read_enable_c` constraint item, so full DV checks are not qualified.
- **Boundary:** Zero-concrete-instance parameterized or array nested children remain explicitly unsupported because their specialization cannot be inferred safely from the available parsed instance metadata. Keep them out of IEEE pass claims; preserve pinned source and all released checks. The unknown CSR field and monitor warnings need separate work after this one-blocker PR.
- **Evidence:** [PR #363](https://github.com/dsellerbrock/iverilog-uvm/pull/363), [original bytecode and RED](../../evidence/opentitan-csrng-nested-vif-handoff-20260924/README.md), and [final focused and pinned replay](../../evidence/opentitan-csrng-nested-vif-fix-20260925/README.md).

### OT-CSRNG-INLINE-RECEIVER-PROPERTY-CONSTRAINT — direct owner-held randomize receiver

- **State:** [PR #366](https://github.com/dsellerbrock/iverilog-uvm/pull/366) recognizes an unindexed owner-held receiver such as `req.randomize() with { req.clen == 12; }` and resolves the direct qualified member against the randomized object. The paired strict 2017/2023 reducer was 0/4 in each legacy/JSON runner on PR #365 and is 4/4 each after the fix. Neighboring tests pass 11/11 each, full legacy 6,617 total with zero unexpected failures, full JSON 3,656/3,656, and `-g2012` real-DPI UVM 358/358.
- **Requirement:** IEEE 1800-2017/2023 §§18.7 and 18.7.1 govern inline target and explicit caller lookup; §§18.6.1 and 18.6.3 govern randomizing the selected object and rollback. The tested direct receiver path preserves target-class name shadowing, distinct caller handles as fixed state, `local::` qualification, and failed-solve rollback; an absent receiver member is rejected.
- **Released DV:** The pinned CSRNG smoke uses explicit nonstandard `-gcommercial-unsafe` and native AES DPI. The previous first-item randomization fatal is absent, and its scoreboard observes one SW-app instantiate command. It remains **0/1 released compatibility DV** at `AesSecCmDataRegKeySca` at 16,454,635 ps with one UVM_ERROR, `TEST FAILED CHECKS`, no completed generate/genbits check, and no pass marker despite VVP exit zero.
- **Boundary and evidence:** Same-object aliases and explicit-`this` caller fallback remain separate strict-language gaps DD-059/60; this fix does not qualify general inline receiver identity or full clause 18. The [revision-scoped commands, focused tests, broad gates, and release replay](../../evidence/opentitan-csrng-inline-receiver-20260925/README.md) preserve the pinned source and checks. The later AES assertion has its own blocker below.

### OT-CSRNG-AES-FINAL-ROUND-STATE-ASSERTION — whole-array assertion reads a late net word

- **State:** [PR #367](https://github.com/dsellerbrock/iverilog-uvm/pull/367) publishes the scoped VVP fix. A paired strict 2017/2023 reducer was RED in both editions: the positive state update and scalar assertion pass, but the equivalent whole-array assertion fails after a net-array input changes later in the antecedent's clock slot. A deliberate bad update fails both assertions. The fix passes the registered 1/1 legacy and 2/2 JSON focus, including second-word, held-force, and strength-resolved net-array controls. Full legacy passes 6,618 total with zero unexpected failures, JSON 3,658/3,658, negatives 155/155, SVA dual-engine 62/62, VPI with PLI1 140/140, and real-DPI UVM 358/358.
- **Requirement and cause:** IEEE 1800-2017 §7.4.3 and 2023 §7.4.6 allow fixed unpacked-array equality; both editions §§16.5.1, 16.9.3, and 16.12.7 require the prior Preponed values in the nonoverlapped consequent. `%hist/on/av` enabled array-owned history, but net-backed words change through their underlying wires and bypass the array's snapshot-before-write path. `%load/preponed/av` then read live words. VVP now enables and reads the existing per-wire first-write history for integral net-array words, while retaining a held force's visible bits and leaving other array storage on its old path.
- **Released DV:** With the pinned OpenTitan source, hash-checked disposable BlkLen overlay, unchanged AES assertion, native AES DPI, and explicit nonstandard `-g2017 -gcommercial-unsafe`, the final private VVP completes the released `csrng_smoke_test` sequence at 77,830,086 ps. The scoreboard observes SW-app instantiate, generate, and uninstantiate; the UVM summary reports zero errors/fatals and `TEST PASSED CHECKS`. This is **1/1 released nonstandard compatibility CSRNG DV**, separate from strict IEEE and UVM regression results.
- **Boundary and evidence:** The compile still warns that `int_state_read_enable_c` is ignored by the constraint solver. That warning and broader CSRNG coverage remain separate; this smoke result does not qualify all OpenTitan DV or the full assertion clause. Same-slot changes to a force overlay need their own history assessment. [Paired RED, exact source/overlay hashes, focused checks, and native-DPI replay](../../evidence/opentitan-csrng-aes-final-round-20260925/README.md) preserve the checks and distinguish the nonstandard release profile.

### OT-OTP-CLASS-SPECIALIZATION-IDENTITY — factory sequencer type identity fixed in PR #360

- **State:** The scoped `elab_scope.cc` change at bb92498ad keys supported concrete multi-value class actuals by effective value. The original factory cast now accepts equal positional/named/default forms and rejects unequal forms in paired 2017/2023 runs. Registered focused legacy and JSON tests pass 6/6 each; the final-image full legacy gate passes 6,587 total with zero failures, VPI 131/131, negatives 155/155, and runtime invariants 15/15. Full JSON passes 3,622/3,622 with zero failures, and real-DPI UVM passes 358/358. Private `ivl` is `57eec3fc…ca423f3`, and the installed Caliptra compiler remains unchanged.
- **Released DV:** Pinned OTP with the unchanged named RAM-path overlay and native DPI clears `cfg.randomize()` and DCLPSQ, then fails `GrantKnown_A` in `prim_arbiter_tree.sv:206` at 1,559,061 ps. One UVM_ERROR, `TEST FAILED CHECKS`, no pass marker, and no meaningful checked traffic make this **0/1 released DV** despite VVP exit zero. The responsible arbiter instance is not yet identified. Its assertion stays enabled; this is a separate blocker for a later PR. CSRNG also remains 0/1 after its nested-VIF handoff clears, on an unknown CSR backdoor field and incomplete monitor elaboration.
- **Boundary and evidence:** The fix changes only the central concrete specialization key and paired tests. A legal dependent-width value formal is still rejected before keying and is recorded as a separate support limit, not an IEEE pass. The pinned OpenTitan source remains clean. The fix is published in [PR #360](https://github.com/dsellerbrock/iverilog-uvm/pull/360). See the [pre-fix factory reducer](../../evidence/opentitan-otp-dd056-factory-registry-20260924/README.md) and [final pinned replay, focus, and gate records](../../evidence/opentitan-otp-class-identity-20260924/README.md).

### CALIPTRA-UNSAFE-VIRTUAL-DISPATCH-REACHABILITY — called task falsely waived (selected P1)

- **State:** Independent paired baseline review found a called interface task reached through a concrete base handle to a virtual `Derived.go` override. The earlier explicit `-gcommercial-unsafe` compiler accepted its overlap with a continuous value-0 drive; VVP drove value 1. This violates the campaign rule that called or uncertain task writes reject. A private elaborate.cc candidate passed [28/28 dedicated and 34/34 neighboring controls](../../evidence/caliptra-unsafe-virtual-dispatch-20260924/README.md), including called virtual rejection and proven-unused allowance. The installed ivl SHA-256 `6ec92c4825dc51e3810d1cb22405071f033015626d39cf3db92c319aeb9d3b10` passes [shared paired P1 focus 28/28](../../evidence/caliptra-unsafe-virtual-dispatch-20260924/shared_focus.json). Its [strict full-top guard](../../evidence/caliptra-icarus-l0-strict-p1-20260924/README.md) still rejects exactly 46 mixed-driver overlaps with zero other errors or Preponed warnings, and launches no runtime. The [P1-install broad legacy gate](../../evidence/caliptra-virtual-final-ivtest-gate-20260924.log) exits zero: 6,555 total, 6,550 passed, zero failed, two not implemented, three expected failures; bundled VPI 131/131, negatives 155/155, and VVP invariants 15/15 also pass. The [final REAL DPI UVM umbrella](../../evidence/caliptra-virtual-final-real-dpi-uvm-20260924.log) passes 358/358 with both VIF smokes. The [full JSON compiler/VVP suite](../../evidence/caliptra-virtual-final-json-gate-20260924.log) passes 3,536/3,536 with zero failures; pristine compatibility review remains; PR #350 merged at c5679aa31.
- **Scope:** The selected implementation may change only `elaborate.cc` and dedicated paired 2017/2023 tests/evidence. Prove reachable overrides are live for the concrete interface instance; retain rejection of uncertain dispatch and genuine overlaps, while preserving the exception for proven-unused task writes. Coordinator owns shared builds, full gates, pinned replay, and review. The [16-case paired 2017/2023 RED](../../evidence/caliptra-interface-driver-reducer-20260923/virtual_override_reachability_result.json) includes called virtual, unused virtual, direct called, and nonvirtual called controls.

### CALIPTRA-L0-INTERFACE-MIXED-DRIVERS — released full top violates variable driver rule

- **Campaign profile:** The selected 52 Caliptra L0 cases use explicit `-gcommercial-unsafe` as the main nonstandard compatibility lane, with the original testbench checks and zero-error gate intact. Strict mode is the focused IEEE guard for genuine mixed-driver overlaps; the full-top strict compile is historical baseline evidence. Named test-selected Caliptra/OpenTitan patches are applied only to disposable copies under the [release overlay guide](release_overlays/README.md), preserving pinned checkouts.
- **State:** Latest P1-installed strict Icarus compilation of the pinned v2.1.2 `caliptra_top_tb` exits 46 on 46 continuous/procedural driver overlaps across two local `axi_if` instances, including with the released internal-TRNG define; it emits zero Preponed sampling warnings and launches 0/52 L0 runtimes. Before PIC integration, an opt-in `-gcommercial-unsafe` candidate compiled the same top after proving that each overlapping task write is unreachable for its concrete interface instance; the existing direct called, unknown, exported, and live-writer controls reject, but the later P1 virtual-dispatch called-task RED exposed an unsafe false waiver in that pre-P1 image. The pre-PIC exact native-vector first compatibility runtime was 0/1 with 51 unrun, 621 instruction-trace entries, simulation exit 1, zero pass/fail markers, and 17,467 bad diagnostics; original TLU, SOC IFC, and LSU assertions act at 43.865 us. Its reported retired-instruction and cycle counters are zero because no final CSR dump was reached. A separate hash-guarded copied-reset diagnostic case was also 0/1. The integrated PIC candidate passes paired focus 4/4. Its initial exact unsafe compile crashed exit 139 on a null `Nexus::first_nlink()` in the new cprop traversal, but a cprop null guard now lets the exact top compile to VVP. The [exact null-guard first-case replay](../../evidence/caliptra-icarus-l0-unsafe-pic-nullguard-first-20260924/smoke_test_veer/result.json) completes with firmware/simulation exits 0, one pass marker, zero fail markers, 633 retired instructions, 4,348 cycles, and 634 trace commits, but 17,863 bad diagnostics fail the unchanged zero-error gate. It is nonstandard compatibility 0/1 with 51 unrun; the former 43.865 us terminal SOC/TLU/LSU fatal is absent. The selected VIF repair now passes the full real-DPI UVM gate 358/358; PIC source work remains suspended while remaining error-root review continues after P1 broad legacy, full JSON 3,536/3,536, and REAL DPI UVM gates passed. The [fresh VIF-install exact unsafe first case](../../evidence/caliptra-icarus-l0-unsafe-vif-first-20260924/README.md) also fails nonstandard compatibility 0/1 with 51 unrun and 17,863 diagnostics despite 633 retired instructions and a pass marker. No strict or pristine compatibility L0 pass is established; the separate combined disposable-copy diagnostic passes one case.
- **Latest profile checkpoints:** The [installed P1 strict summary](../../evidence/caliptra-icarus-l0-strict-p1-20260924/summary.json) has compile exit 46, 46 genuine overlaps, zero other errors or Preponed warnings, and 0/52 runtimes attempted. The [bundled-BFM reset-copy summary](../../evidence/caliptra-icarus-l0-diagnostic-reset-vif-20260924/summary.json) is `diagnostic_reset_overlay` 0/1 with 51 unrun, 633 retired instructions, one pass marker, zero startup errors, exactly 17,844 checker prints, and clean pinned sources/fingerprints. The [combined reset/checker disposable-copy summary](../../evidence/caliptra-icarus-l0-diagnostic-combined-p1-20260924/summary.json) passes `diagnostic_reset_checker_source_overlay` 1/1 with 51 unrun, zero bad diagnostics, one pass marker, 633 retired instructions, 4,348 cycles, and 634 trace commits; this is separate from pristine unsafe 0/1 and IEEE conformance. The runner now uses per-case firmware source roots, and a focused check across all 52 selected names and frozen patch hashes checks each named patch target, keeps the other 50 on the pinned source root, and rejects cross-target patch-target tampering. Existing one-case results are unaffected. The preliminary sequential `--all` diagnostic run was intentionally stopped during `smoke_test_mbox` after `smoke_test_veer` printed PASS; preserved partial output at `evidence/caliptra-icarus-l0-diagnostic-combined-all-p1-20260924/` has no aggregate result and is unqualified. [During/after provenance](../../evidence/caliptra-l0-active-sweep-provenance-20260924/README.md) confirms both 5,139-entry copied firmware trees differ from pinned source only by their respective named patch and xPack GCC 15.2 toolchain hashes remained unchanged. A new ephemeral-JTAG-port profile passes focused copy and bind-error checks but has no runtime; a bounded-parallel wrapper is pending.
- **Requirement:** IEEE 1800-2017/2023 §§6.5 and 10.3.2 prohibit a procedural assignment to a variable also driven continuously. The 23 assignments per instance in `caliptra_top_tb_axi_complex.sv` overlap task-body assignments declared by `axi_if`. The existing paired legal forwarding, direct-overlap, direct-called-task, and uncalled-task controls agree between Icarus and Slang in both editions; they did not cover the virtual-dispatch called path now selected as a P1 soundness breach. No unmodified release configuration avoids this conflict: globally excluding those tasks leaves active `m_axi_bfm_if.rst_mgr`, read, and write calls unresolved. A release-side separation of the task-bearing BFM interface from the continuously forwarded internal interfaces, or otherwise a single legal driver per member, is needed. No standards-correct elaborator fix is established for the released source shape.
- **Evidence:** [Latest integrated PIC strict replay](../../evidence/caliptra-icarus-l0-strict-pic-nullguard-20260924/summary.json), [pre-PIC unsafe first-case result](../../evidence/caliptra-icarus-l0-unsafe-final-first-20260924/smoke_test_veer/result.json), [paired driver reducers](../../evidence/caliptra-interface-driver-reducer-20260923/README.md), [copied-reset post-AES runtime](../../evidence/caliptra-icarus-l0-diagnostic-aesfix-first-20260923/summary.json), [port-binding reducer](../../evidence/caliptra-ahb-vif-anyedge-20260923/README.md), [reset-race reducer](../../evidence/caliptra-l0-timezero-reset-overlay-20260923/README.md), and [AES sensitivity fix](../../evidence/caliptra-aes-sensitivity-fix-20260923/README.md). Pre-PIC shared installation passes driver 34/34 and interface-port/ref 28/28 paired cases in each legacy and JSON harness. The pre-PIC full legacy gate passes 6,551 total with zero failures, plus VPI 131/131, negatives 155/155, and VVP invariants 15/15; pre-PIC JSON gate passes 3480/3480. The pre-PIC real-DPI UVM umbrella loaded DPI but failed 356/358 with `vif_smoke` and `vif_smoke_v2` compile diagnostics on `count`; the narrow elaborate.cc VIF repair now passes the [shared real-DPI UVM gate](../../evidence/caliptra-vif-final-real-dpi-uvm-20260924.log) 358/358 with zero failures/skips and both smokes passing. The first VIF-install legacy gate failed one `sv_packed_mixed_driver_compound` expected-warning gold mismatch (6,555 total; 6,549 pass, two not implemented, three expected failures), while VPI 131/131, negatives 155/155, and invariants 15/15 pass. Its corrected gold passes focused legacy 1/1 and JSON 2/2, but the broad rerun was intentionally stopped after the P1 unsafe-dispatch RED; the [P1-install full JSON gate](../../evidence/caliptra-virtual-final-json-gate-20260924.log) passes 3,536/3,536 with zero failures, while PR qualification remains open. The UVM result does not establish a Caliptra L0 pass. Preserve the strict diagnostic and every L0 checker; keep compile-blocked cases separate from runtime failures and the separate `csrng_tb` unit run outside the 52-test numerator. Pristine explicit unsafe results are nonstandard compatibility evidence, while copied-testbench or checker-patched runs carry distinct diagnostic qualifications; none is IEEE conformance.

### CALIPTRA-VIF-DISJOINT-PROPERTY-WRITER — legal VIF reset write falsely overlaps count output

- **State:** Selected after the pre-PIC real-DPI UVM umbrella loaded real DPI but failed 356/358. Before the repair, both `vif_smoke.sv:96` and `vif_smoke_v2.sv:96` failed strict and explicit unsafe compilation with a continuous/procedural-driver error on `count`, although the VIF procedural write targets `reset` and the DUT output drives disjoint ordinary `PWire` member `count`. Temporary paired 2017/2023 REDs reproduce the false overlap. The shared installed elaborate.cc candidate passes [paired focus 20/20](../../evidence/caliptra-vif-disjoint-property-writer-20260924/shared_new_focus.log) and [neighbor controls 16/16](../../evidence/caliptra-vif-disjoint-property-writer-20260924/shared_neighbor_focus.log). Both real-DPI VIF smokes previously compiled/ran privately with `PASS counter_test` and zero UVM error/fatal. Installed iverilog/ivl/vvp SHA-256 prefixes are 1590b064/ba1dda3d/e9b7a64a. The [full shared real-DPI UVM gate](../../evidence/caliptra-vif-final-real-dpi-uvm-20260924.log) now passes 358/358 with zero failures/skips and both `vif_smoke` variants passing. The [fresh VIF-install first case](../../evidence/caliptra-icarus-l0-unsafe-vif-first-20260924/README.md) fails nonstandard compatibility 0/1 with 51 unrun and 17,863 diagnostics. The first broad legacy gate had one expected-warning gold mismatch; focused retest passes, but its rerun was intentionally stopped after the separate P1 unsafe-dispatch discovery. P1-install broad legacy, full JSON 3,536/3,536, and REAL DPI UVM gates now pass; pristine compatibility review remains pending. This earlier VIF UVM result is separate from Caliptra L0.
- **Requirement and hypothesis:** Distinct ordinary interface members are distinct variables. Exact same-member overlap, uncertain VIF receiver, modport alias, and same-member or uncertain port/ref aliases must still reject in strict and unsafe modes. A known-disjoint `ref` actual is a separate legal case that the current tool still falsely rejects. The installed direct-property repair resolves the concrete receiver/member false overlap. The known-disjoint `ref` actual remains separate false-rejection debt; the P1 virtual-dispatch repair now passes installed paired focus 28/28 and broad legacy, JSON, and REAL DPI UVM gates without widening `-gcommercial-unsafe` waivers.
- **Scope and evidence:** The implementation agent owns `elaborate.cc` plus dedicated paired 2017/2023 ivtest fixtures/golds/evidence only; coordinator owns shared build/install, manifests, the full real-DPI UVM rerun, and broader qualification. The [pre-VIF-fix failing real-DPI UVM gate](../../evidence/caliptra-final-real-dpi-uvm-20260924.log) is the application baseline; the [current shared gate](../../evidence/caliptra-vif-final-real-dpi-uvm-20260924.log) passes 358/358. PIC source work remains suspended after its earlier completed first case failed 0/1 on 17,863 diagnostics. The fresh VIF-install case also fails 0/1; P1-install broad legacy, full JSON 3,536/3,536, and REAL DPI UVM gates pass; pristine compatibility review remains pending, while error-root triage stays open. Pinned sources and all L0 checks stay intact.

### CALIPTRA-VIF-DISJOINT-REF-ACTUAL-FALSE-REJECT — known-disjoint ref member remains rejected

- **State:** Independent paired 2017/2023 review shows `write_ref(vif.reset)` with a continuous `bus.count` drive is rejected in strict and unsafe modes. Direct `vif.reset` compiles, and a same-member `ref` actual correctly rejects. The disjoint `ref` error predates the selected VIF member-name repair and does not block the released Caliptra L0 path.
- **Boundary:** Unresolved `ref` actuals lose `property_idx` before driver analysis. Treat this as separate follow-on debt; no source fix is selected in the current lane. Preserve same-member and uncertain-alias rejection, the narrow unsafe waiver, pinned sources, and all L0 checks. The [paired strict/unsafe reducer](../../evidence/caliptra-vif-disjoint-property-writer-20260924/ref_member_identity_result.json) records legal direct disjoint, false-rejected disjoint `ref`, and correctly rejected same-member `ref` controls in both editions.

### CALIPTRA-AES-FUNCTION-LOCAL-SENSITIVITY — automatic function locals trigger other `always_comb` callers

- **State:** The narrow interval-aware sensitivity subtraction passes 8/8 paired 2017/2023 focused cases, plus 10/10 legacy and 8/8 JSON synthesis neighbors. The pre-fix two-caller automatic-function reducer spun at time zero; the pre-PIC corrected full top advances through 621 instruction-trace entries before separate pinned TLU, SOC IFC, and Veer assertions fail. The sensitivity defect is locally fixed, but no L0 pass is established.
- **Root cause and requirement:** `NetBlock::nex_input` precisely records constant bit writes but `NexusSet::rem` subtracts only exactly matching ranges, leaving a whole-vector read of the fully written local in sensitivity. IEEE 1800-2017/2023 §9.2.2.2.1 excludes written expressions from `always_comb` implicit sensitivity while retaining reads of unwritten bits. A dedicated interval-aware subtraction is selected; exact event-expression matching must remain unchanged.
- **Evidence:** [Paired reducer and VVP comparison](../../evidence/caliptra-aes-timezero-20260923/README.md), [focused fix](../../evidence/caliptra-aes-sensitivity-fix-20260923/README.md), and [post-fix diagnostic runtime](../../evidence/caliptra-icarus-l0-diagnostic-aesfix-first-20260923/summary.json). Preserve the pinned sources and every L0 check. A checked first-case pass is required before expanding to 52.

### CALIPTRA-NESTED-PACKED-PREPONED-SAMPLING — constant packed selects sampled live

- **State:** The local nested constant packed-select fix passes paired 2017/2023 focus 2/2 in each legacy and JSON harness. The final installed strict full-top compile emits zero Preponed sampling warnings where its predecessor emitted 32; the top still fails on 46 mixed drivers, so this establishes no L0 runtime pass. The pre-VIF-fix full legacy gate passes; JSON passes 3480/3480 and pre-VIF-fix real-DPI UVM failed 356/358 on separate VIF smoke compile diagnostics. The current shared VIF installation passes real-DPI UVM 358/358; the installed P1 repair passes paired focus 28/28, while fresh P1-install broad legacy, full JSON 3,536/3,536, and REAL DPI UVM gates pass.
- **Requirement and boundary:** Concurrent assertion operands must use their Preponed values. The previous elaboration left nested constant packed selections on a live-value path, which could produce false assertion failures. The fix reconstructs the nested constant selection and retains its typed select metadata. Dynamic nested-index sampling remains unqualified.
- **Evidence:** [Paired RED, candidate and boundary](../../evidence/caliptra-packed-struct-preponed-20260924/README.md) and [latest strict compile](../../evidence/caliptra-icarus-l0-strict-pic-nullguard-20260924/summary.json). The L0 runner treats a remaining sampling warning as disqualifying.

### CALIPTRA-POST-AES-INTERRUPT-ASSERTIONS — first firmware case fails after interrupt enable

- **State:** The pre-PIC exact native-vector `-gcommercial-unsafe` first case was 0/1 at 43.865 us after 621 instruction-trace entries. The Veer TLU fast-interrupt flush assertion reports first; SOC IFC `ERR_HWIF_IN` calls `$fatal`, and the Veer store-buffer assertion reports in the same terminal interval. The separate copied-reset diagnostic case has the same 0/1 outcome. None is a pass. A third hash-guarded copied-source probe observes unknown PIC interrupt input before that interval; a tiny paired 2017/2023 reducer copied from the pinned PIC priority tree gives `selected_priority=zzzz` from 32 known-zero leaves, proving an isolated simulator defect. The full runtime causal relation remains open. The initial integrated PIC exact unsafe compile crash was repaired by a cprop null guard. The first case now completes without the 43.865 us terminal SOC/TLU/LSU fatal, but its 17,863 diagnostics disqualify the pass banner and simulation exit 0; nonstandard compatibility remains 0/1.
- **Boundary:** The preceding MLDSA/KV `SVA ERROR` details arise from eager checker-function evaluation under false property antecedents in a paired reducer, without an observed outer assertion action. IEEE 1800-2017/2023 §16.6 disallows side effects in assertion-expression functions; the pinned KV/MLDSA checker functions print internally. A [read-only paired assessment](../../evidence/caliptra-sva-eager-assessment-20260924/README.md) shows pure-property truth and exactly one true consequent failure in the direct and legal pure controls, while the impure function is called five times. §16.12.7 conditions consequent evaluation on antecedent match, but this illegal source shape does not prove an assertion-verdict defect or authorize an implementation. Every printed error still violates the L0 zero-error rule. A paired 2017/2023 test of the actual 2,422-bit SOC IFC packed input type shows `$isunknown` agrees with a per-bit check for all-known and one-X controls, ruling out that isolated false-positive mechanism.
- **Evidence:** [Exact first-case result](../../evidence/caliptra-icarus-l0-unsafe-final-first-20260924/smoke_test_veer/result.json), [assertion/action triage, copied-source probes, and PIC priority reducer](../../evidence/caliptra-post-aes-runtime-fatal-triage-20260923/README.md), [checker side-effect reducer](../../evidence/caliptra-post-aes-sva-triage-20260923/README.md), [source-legality assessment](../../evidence/caliptra-sva-eager-assessment-20260924/README.md), and [packed-type `$isunknown` control](../../evidence/caliptra-post-aes-hwif-unknown-20260923/results.json). Preserve the pinned sources and every L0 checker. The separately selected PIC fix below has a narrow cprop.cc scope.

### CALIPTRA-TIMEZERO-ASSERTION-READ-ONLY-DEBT — early reset-dependent assertion diagnostics

- **State:** The unchanged-BFM first case has 19 startup actions: 14 Veer time-zero deferred-immediate failures, two Adams Bridge first-edge immediate errors, and three DOE cold-reset SVA failures. A [six-nanosecond passive VPI trace](../../evidence/caliptra-l0-timezero-passive-vpi-20260924/README.md) records unknown reset-dependent checker inputs before the first clock; it does not prove whether Icarus scheduling or released testbench ordering caused them. A hash-guarded copy of the **bundled Caliptra BFM** shifts its initial power-good drive by `#0` and removes all 19 startup actions. The completed [reset-copy first case](../../evidence/caliptra-icarus-l0-diagnostic-reset-vif-20260924/README.md) is still `diagnostic_reset_overlay` **0/1** with 51 unrun, one pass marker, 633 retired instructions, and exactly 17,844 vacuous KV/MLDSA checker messages as its only bad diagnostics.
- **Boundary:** Read-only causal assessment and copied-BFM diagnostic; no selected compiler fix, assertion suppression, pristine L0 pass, or IEEE qualification follows. Pinned sources, fingerprints, and released checkers remain intact. Keep the reset-copy denominator separate from pristine `-gcommercial-unsafe` nonstandard compatibility.

### CALIPTRA-EAGER-CONSEQUENT-CHECKER-DIAGNOSTICS — read-only compiler-source triage

- **Observation:** In the [PIC-install first-case simulation](../../evidence/caliptra-icarus-l0-unsafe-pic-nullguard-first-20260924/smoke_test_veer/result.json), 4,461 KV and 13,383 MLDSA checker-function `SVA ERROR` lines account for 17,844 of 17,863 bad diagnostics. The other 19 are 16 early plain errors and three reset-SVA errors before or at the first 5 ns edge; no later other errors were recorded. All 17,863 disqualify the case under the unchanged zero-error rule. The [fresh VIF-install replay](../../evidence/caliptra-icarus-l0-unsafe-vif-first-20260924/README.md) independently fails 0/1 with the same 16 + 3 + 17,844 decomposition.
- **Boundary:** [Paired 2017/2023 pure/impure controls](../../evidence/caliptra-sva-eager-assessment-20260924/README.md) trace the eager calls to `pform.cc` consequent-expression capture before the observed wait. The pinned checker functions print internally and violate the assertion-expression side-effect restriction in §16.6, so these messages alone do not prove a legal assertion-verdict defect. No compiler-source implementation is selected. The separate four-helper disposable source overlay below has paired controls and keeps the remaining real assertion diagnostics and L0 zero-error gate intact.

### CALIPTRA-L0-CHECKER-SOURCE-OVERLAY — four impure helpers on a disposable copy

- **State:** A [frozen four-helper patch and paired controls](../../evidence/caliptra-l0-checker-source-overlay-20260924/README.md) remove nine internal detail `$display` calls from the KV and MLDSA secret-key, public-key, and signature helpers on a disposable copy of pinned `caliptra_top_sva.sv`. All comparisons, returns, property expressions, disable guards, and outer failure actions remain byte-for-byte. Paired 2017/2023 pure/impure vacuous, true-pass, true-fail, reset-disable, and same-slot NBA controls pass [4/4](../../evidence/caliptra-l0-checker-source-overlay-20260924/results.json) with identical outer actions and zero versus 18 eager inner prints. The opt-in full-top compile exits zero with zero sampling warnings; the [combined reset/checker first case](../../evidence/caliptra-icarus-l0-diagnostic-combined-p1-20260924/README.md) passes diagnostic qualification 1/1 with 51 unrun, zero bad diagnostics, one pass marker, and 633 retired instructions.
- **Boundary:** This is a selected coordinator-owned **diagnostic source-copy overlay**, disjoint from the P1 `elaborate.cc` compiler repair. The runner records patch/pre/post hashes and separate `diagnostic_checker_source_overlay` or `diagnostic_reset_checker_source_overlay` qualification. The reset-only diagnostic is 0/1 with 51 unrun and exactly 17,844 checker prints; neither it nor the completed combined diagnostic 1/1 enters the pristine nonstandard compatibility numerator. First-mismatch internal detail is lost, while unchanged outer actions and all other L0 checks remain. The combined one-case pass is a diagnostic source-copy result only. The preliminary sequential `--all` output was stopped and has no aggregate result; no pristine or 52-case L0 pass is claimed.

### CALIPTRA-L0-EPHEMERAL-JTAG-PORT-DIAGNOSTIC — isolated listener for bounded replays

- **State:** The runner's explicit `--ephemeral-jtag-port` uses a hash-guarded disposable copy of only the pinned top testbench, changing its sole JTAG `ListenPort` from `63224` to `0`. A [focused copy and bind-error gate](../../evidence/caliptra-icarus-l0-runner-20260923/check_ephemeral_jtag_port.py) passes. The runner rejects JTAG server/bind diagnostics even with a pass marker. With unsafe, the copied bundled Caliptra BFM reset overlay, and the four-helper checker source overlay, the separate label is `diagnostic_reset_checker_source_ephemeral_jtag_port_overlay`. The [same-profile first case](../../evidence/caliptra-icarus-l0-ephemeral-jtag-first-20260924/README.md) passes diagnostic qualification **1/1** with 51 unrun, firmware/VVP exits zero, one pass marker, zero bad/DPI/JTAG diagnostics, 633 retired instructions, 4,348 cycles, and 634 trace commits; source/tool integrity matches. The [bounded four-job exact 52-case wrapper](../../evidence/caliptra-icarus-l0-parallel-runner-20260924/README.md) was deliberately SIGTERM-stopped after one completed `smoke_test_veer` PASS. Longer first-cohort cases were still in released CRT `.data` copy loops; static map/disassembly shows about 17,700-17,900 instructions to `main` versus about 2,400 after 22 minutes, so the 1,800-second limit cannot reach `main`. Its partial output at `evidence/caliptra-icarus-l0-patched-52-20260924/` is unqualified, with no aggregate result or orphan VVP. A same-profile `smoke_test_mbox` pilot with `--timeout 14400` is active at `evidence/caliptra-icarus-l0-mbox-long-pilot-20260924/`.
- **Boundary:** Pinned sources and every L0 check remain unchanged. The stopped sequential partial output at `evidence/caliptra-icarus-l0-diagnostic-combined-all-p1-20260924/` belongs to the prior `diagnostic_reset_checker_source_overlay` profile and has no aggregate verdict. [During/after source-copy and xPack toolchain attestation](../../evidence/caliptra-l0-active-sweep-provenance-20260924/README.md) applies to that stopped run; it does not qualify the new-profile 1/1 result or IEEE conformance. The new-profile first case is also separate from the pristine unsafe denominator; no 52-case pass is claimed.

### CALIPTRA-PIC-PACKED-PARTIAL-DRIVER-CYCLE — dependent packed partial drivers form a self-cycle

- **State:** Selected for one narrow cprop.cc implementation lane after paired 2017/2023 PIC RED and passing independent disjoint-driver and whole-array controls. The installed RED yields `selected_priority=zzzz` from 32 known-zero leaves. A private bounded-reachability cprop candidate passes the paired PIC tree and dependent-slice cases; a partially uncovered 3-bit whole-vector feedback RED changes baseline `zzz` to candidate `z11` in both editions, with a nonfatal target `uwire` multi-driver warning. Integrated paired PIC focus passes 4/4. Its first exact unsafe compile crashed exit 139 from a null `Nexus::first_nlink()` in the new cprop reachability traversal; an integrated null guard now compiles the exact top to VVP. The [exact null-guard first-case replay](../../evidence/caliptra-icarus-l0-unsafe-pic-nullguard-first-20260924/smoke_test_veer/result.json) completes with firmware/simulation exits 0, one pass marker, zero fail markers, 633 retired instructions, 4,348 cycles, and 634 trace commits. It is still nonstandard compatibility 0/1 with 51 unrun because 17,863 bad diagnostics violate the zero-error gate. Removal of the prior terminal 43.865 us SOC/TLU/LSU fatal is a material runtime advance, not an L0 pass; the pre-PIC unsafe 0/1 remains historical and separate. The latest strict top still exits 46 with 46 mixed-driver errors and zero Preponed warnings, so strict L0 remains 0/52 runtimes attempted. The prerequisite real-DPI UVM gate now passes 358/358; PIC source work remains suspended while error-root triage continues after P1 broad legacy, full JSON 3,536/3,536, and REAL DPI UVM passes.
- **Requirement and cause:** Disjoint nested packed continuous assignments must drive their selected bits without feeding the destination into itself. `PGAssign::elaborate` to `PEIdent::elaborate_lnet_common_` creates `NetPartSelect::PV`; `cprop_functor::lpm_part_select` in `cprop.cc:207-347` blends PV drivers into `NetConcat` despite a combinational path from destination structural inputs back to a candidate PV driver input. Direct VP and whole-vector reduction paths show why a VP-only guard is insufficient. The selected cprop guard checks bounded combinational reachability and stops at flip-flops; independent disjoint drivers, two-state, registered, and unrelated-reader controls must retain their behavior.
- **Boundary and evidence:** A separate fully covered 2-bit whole-vector feedback RED remains `zz` instead of `11` because `elab_net.cc`/`netmisc.cc:3170` `collapse_partselect_pv_to_concat` forms the cycle during elaboration. That path is recorded debt, outside this selected cprop.cc source scope; no full packed-driver qualification is claimed. The coordinator owns shared builds, installation, manifests, integration and pinned replay. [PIC RED and controls](../../evidence/caliptra-post-aes-runtime-fatal-triage-20260923/pic_priority_tree_reducer.sv), [source-shape reducer](../../evidence/caliptra-post-aes-runtime-fatal-triage-20260923/pic_packed_slice_reducer.sv), and [paired partial whole-read RED](../../evidence/caliptra-post-aes-runtime-fatal-triage-20260923/packed_whole_read_cycle_min.sv) establish the scoped mechanism; all original L0 assertions remain active. [Integrated paired focus](../../evidence/caliptra-pic-integrated-focus-20260924.log), [intermediate strict compile](../../evidence/caliptra-icarus-l0-strict-pic-20260924/summary.json), and [intermediate unsafe compile crash](../../evidence/caliptra-icarus-l0-unsafe-pic-first-20260924/summary.json) record the caught candidate issue. The [final strict compile](../../evidence/caliptra-icarus-l0-strict-pic-nullguard-20260924/summary.json) and [completed unsafe compatibility replay](../../evidence/caliptra-icarus-l0-unsafe-pic-nullguard-first-20260924/summary.json) record the installed null-guard checkpoint.

### CALIPTRA-L0-DOE-STATUS-SVA — Verilator retains a pre-reset DOE assertion attempt

- **State:** The full selected pinned Caliptra v2.1.2 / Adams Bridge v2.0.3 Verilator L0 replay is 51/52, zero timeouts; `smoke_test_doe_scan` alone emits one SVA error despite exit 0 and a pass banner. The released DOE property is unchanged.
- **Cause and evidence:** The [passive released timing trace](../../evidence/caliptra-doe-root-cause-20260923/assessment.md) shows `clear_obf_secrets=1` sampled on RDC edges before reset, DOE reset pulsing low before the consequent, and `clear_obf_secrets=0` at the first resumed RDC edge. IEEE 1800-2017/2023 §16.12 requires the pending attempt to be aborted by `disable iff`; Verilator checks the stale direct-`|=>` obligation anyway. The [reducer and restored v5.052 source diagnosis](../../evidence/caliptra-doe-verilator-midwindow-20260923/assessment.md) isolate that simulator behavior. Paired Icarus causal controls abort the pre-reset attempt and detect a deliberately fresh post-reset failure.
- **Boundary:** This is a Verilator L0 diagnostic, not an Icarus DOE DV pass or a full Caliptra qualification. No assertion suppression or pinned-source change is justified. A corrected simulator must rerun the unchanged DOE test with zero SVA errors; Verilator implementation is outside this campaign's selected scope.
...
- **Boundary:** Fixed class-property slice-to-queue assignments at scoreboard lines 1373–1377, virtual-interface array marshalling, and strict mixed-driver overlaps remain separate blockers; packed-parameter `foreach` is addressed below. Multiple ordinary stream operands containing a slice and a pre-sliced operand followed by `with` remain outside this increment.

### OT-OTP-PACKED-PARAM-FOREACH — focused fix, release compile still blocked

- **Scope:** A resolved packed parameter now iterates over its declared indices using the existing static `foreach` loop builder. Sign-only and untyped integral parameters use their inferred logic-vector range; explicit scalar and built-in atom targets still reject (IEEE 1800-2017/2023 §§6.20.2, 12.7.3).
- **Evidence:** [Paired reducers, focused checks, and pinned replay](../../evidence/opentitan-otp-packed-param-foreach-20260924/README.md). Pinned `otp_ctrl_sim` loses its `FATAL_EXP_STATUS` line-294 error but still exits 16 with 15 explicit errors. `chip_sim` retains 48 explicit errors and its existing sensitivity assertion abort. Neither has DV runtime from these compiles.
- **Boundary:** Fixed class-property slice-to-queue assignments are addressed below; virtual-interface array marshalling and strict mixed drivers remain separate OTP blockers.

### OT-OTP-FIXED-SLICE-TO-QUEUE — focused fix, release compile still blocked

- **Scope:** A one-dimensional fixed unpacked-array slice assigned to a plain queue now copies selected packed-integral elements in declared order. Direct arrays and simple class properties are supported; bounds, direction, element type, local/protected declaring-class visibility, and modport access are checked. A packed scalar part-select no longer silently converts to a queue (IEEE 1800-2017/2023 §§7.4.6, 7.6, 8.18).
- **Evidence:** [Paired reducers and pinned replay](../../evidence/opentitan-otp-fixed-slice-queue-20260924/README.md). Pinned `otp_ctrl_sim` loses all five `otp_ctrl_scoreboard.sv:1373-1377` errors but still exits 11 with 10 explicit mixed-driver errors. `chip_sim` retains 48 explicit errors and its existing sensitivity assertion abort. Neither has DV runtime from these compiles.
- **Boundary:** Simple blocking queue targets only; object-backed elements and selections above 65,536 elements reject loudly. Independent review found pre-existing external protected/inherited local visibility false acceptance in scalar and whole-array property reads; this fix rejects those cases on its new slice path only. Remaining OTP strict mixed drivers and chip sensitivity assertion require separate root-cause work.

### OT-ALERT-ESC-CLOCKING-OUTPUT-ALIAS — focused fix, release compile still blocked

- **Scope:** A virtual clocking-output write to a declared, disjoint raw member no longer falsely conflicts with a continuous assignment to another member. Output aliases use their declared raw target; unresolved mappings stay conservative. Successful static and same-scope output writes to the continuously driven member now reject, while an unwritten clocking-output declaration stays legal. The same-member rejection also holds with `-gcommercial-unsafe` (IEEE 1800-2017/2023 §§10.4.2, 14.4, 14.16).
- **Evidence:** [Paired reducers and pinned replay](../../evidence/opentitan-alert-clock-output-driver-20260924/README.md). The pinned `otp_ctrl_sim` loses ten `alert_esc_if.sv` mixed-driver errors and reaches one unsupported fixed unpacked-array virtual-interface task argument at `otp_ctrl_base_vseq.sv:411` (compile exit 1, no explicit errors). `chip_sim` retains 48 explicit errors and its existing `entropy_src_core.sv:3324` sensitivity assertion abort. Neither has DV runtime.
- **Boundary:** Only proven clocking-output identity is treated as disjoint; unknown or colliding generated-property names remain conservative. Virtual-interface fixed-array task marshalling and the chip sensitivity assertion are separate blockers. Caliptra's selected 52 still use explicit `-gcommercial-unsafe` as a nonstandard compatibility profile, with their original checks intact.

### OT-OTP-VIF-FIXED-INPUT — focused fix, OTP compiles but DV fails at time zero

- **Scope:** A class-held virtual interface can copy a compatible, whole one-dimensional packed-integral fixed unpacked array into the selected input task. Automatic and static task runtime tests check selected instance, declared order, four-state values, and caller isolation. Incompatible size/type/scalar actuals reject; omitted/default arrays, output/inout/ref arrays, multidimensional arrays, and fixed-array functions remain loud limitations (IEEE 1800-2017/2023 §§7.6, 13.5.1, 25.9).
- **Evidence:** [Paired reducers and pinned replay](../../evidence/opentitan-otp-vif-fixed-input-20260924/README.md). Private ivl `302d7094` passes paired 22/22, registered JSON 13/13, legacy 4/4, neighboring VIF JSON 44/44, negatives 155/155, and real-DPI UVM 358/358. Pinned `otp_ctrl_sim` compile exits zero with no hard errors or assertions, but 46 nonfatal `sorry` diagnostics remain (six dropped covergroup bins and 40 procedural-continuous-assignment warnings). Its released smoke still fails DV at time zero: `mem_bkdr_util.sv:76` rejects an OTP RAM path string with 18 leading spaces; there is no meaningful traffic or pass marker. `chip_sim` remains a compile failure with 48 explicit errors and one sensitivity assertion.
- **Boundary:** The pinned memory-path macro formatting and chip sensitivity are separate blockers. No OpenTitan application pass or full §§7.6/13.5.1/25.9 qualification is claimed.

### OT-OTP-BACKDOOR-PATH-COMPAT — RAM path repaired; next cfg blocker exposed

- **Failure:** The first released OTP smoke fails at time zero in `mem_bkdr_util.sv:76`: `uvm_hdl_check_path` receives 18 leading spaces before an otherwise existing OTP RAM hierarchy. VVP exits zero after a UVM fatal and 512 follow-on assertions, without meaningful DV traffic or a pass marker. [Exact compile, runtime, and path provenance](../../evidence/opentitan-otp-vif-fixed-input-20260924/README.md).
- **Fix and evidence:** The [hash-checked disposable overlay](release_overlays/opentitan/otp_mem_path_overlay.py) flattens only those two macro bodies. [Paired 2017/2023 controls and the exact pinned replay](../../evidence/opentitan-otp-backdoor-path-overlay-20260924/README.md) confirm the emitted path loses the 18 spaces and the RAM backdoor fatal clears, while hierarchy, `$size`, `$bits`, ECC, and the fatal check remain. Compile exits zero with no hard errors but still emits 46 nonfatal `sorry` diagnostics, including dropped coverage bins, and warns that a constraint item is ignored. These remain separate qualification gaps.
- **Remaining failure:** The released `otp_ctrl_smoke_vseq` is 0/1 DV at time zero: `otp_ctrl_env_cfg.randomize()` hits the existing unresolved joint-distribution random-guard restriction, followed by one UVM_FATAL and 512 end-of-simulation assertion errors. There is no pass marker or meaningful traffic despite VVP exit zero. Paired nested-object reducers reproduce that distinct solver failure in both editions; next work belongs to `OT-CSRNG-CFG-JOINT-DIST`.
- **Boundary:** This is a release-compatibility path, not an IEEE macro-conformance result or an OpenTitan DV pass. The pinned checkout and shared installed tools remain unchanged while Caliptra runs.

### OT-OTP-PACKED-DRIVER-CHAIN — arbiter grant no longer starts unknown

- **Failure:** Legal disjoint packed-bit continuous drivers can read earlier bits of the same vector. The eager `elab_net.cc` collapse turns them into a self-referential transparent concat before their right-hand sides exist; VVP then never receives all concat inputs, leaving even a constant root at Z. Paired 2017/2023 three-bit reducers fail before the fix, and pinned `otp_ctrl_smoke_vseq` fails `GrantKnown_A` with known requests and ready.
- **Fix:** Keep the part drivers until `cprop.cc` can see their dependencies. Its existing feedback guard retains dependent drivers; fully covered independent drivers still use a strength-preserving transparent concat where the target supports it. The SDF interconnect lookup follows a retained scalar part driver only when deferred links prove it is the sole structural driver of that bit; invalid indexed ports are rejected before annotation. Paired legal chains now propagate at startup and after input changes; paired overlapping-variable negatives still reject. [Reproducer, commands, and release replay](../../evidence/opentitan-otp-packed-driver-chain-20260925/README.md).
- **Released DV boundary:** The unchanged OTP smoke reaches its pass marker with balanced checked TL traffic and no UVM errors, but remains **0/1 qualified DV**. Icarus drops the smoke's partition-specific DAI legal-address constraint; a verbose replay shows two out-of-range DAI addresses. Six covergroup bins are also dropped, and the legal packed-variable reducer still emits a “treated as wire” warning. These are separate conformance and DV blockers, not a qualification of this run.

### OT-OTP-DAI-ADDRESS-CONSTRAINT-CONSTANT-MEMBER — fixed ranges retained

- **Failure:** The pinned OTP smoke's eleven conditional `dai_addr inside` ranges use selected members of a package `localparam` array of packed structs. Class-constraint lowering discarded every range before solver translation, leaving only `solve part_idx before dai_addr`. Paired 2017/2023 reductions accepted out-of-range addresses and silently ignored a nonexistent struct member.
- **Fix:** After proving a lexical parameter root and a constant index under constraint lookup, `elaborate.cc` reuses ordinary parameter-member elaboration. It resolves the full path before any same-named terminal class property and diagnoses selected members with meaningful bits above bit 63 rather than truncating them. Paired endpoint, adjacent-invalid, rollback, shadow, explicit-package, invalid-member, and wide-value controls pass. [Commands and pinned replay](../../evidence/opentitan-otp-dai-param-member-20260925/README.md) record the private image and separate gate results.
- **Released DV boundary:** The pinned native-DPI smoke completes with `TEST PASSED CHECKS`, zero UVM/assertion errors, 1343 checked TL A/D pairs, and one DAI address `0x658` inside partition 4. It remains **0/1 qualified released DV** because the distinct rand-indexed `PartInfo[part_idx].secret` condition in `dai_wr_blank_addr_c` is still ignored, as are six covergroup bins. The pinned application checkout and all original checks remain unchanged; unsupported checks are recorded rather than credited as passing.

### OT-OTP-COVER-BIN-METHOD-ENDPOINT — six OTP bins retained

- **Failure:** The pinned OTP `csr_rd_after_alert_cg` lost six bins whose 36 range endpoints call `ral.<register>.get_offset()` through a covergroup constructor handle. Paired 2017/2023 direct and signed reducers compiled with a dropped-bin diagnostic and failed at runtime.
- **Fix:** Capture supported method endpoints once per covergroup instance before VVP resolves dynamic bins. Strict mode proves direct non-ref, constant-state methods and their effective defaults; unsupported nested, ref, mutable, unknown, or unfixed forms reject loudly. The pinned nested RAL path requires an explicit `-gcommercial-unsafe` warning and is nonstandard because method purity is unproved. Side-effecting getters remain possible under that opt-in flag. [Paired reducers and pinned replay](../../evidence/opentitan-otp-cover-bin-method-20260925/README.md) record the exact scope.
- **Released DV boundary:** The named `otp_ctrl_smoke_vseq` passes **1/1 nonstandard compatibility DV** on the final private image: six bin groups and all 36 endpoints are retained, no constraint item is ignored, runtime completes with zero UVM/assertion failures and 1343 checked TL A/D pairs. This seed does not prove after-alert bin hits or the 40 preexisting fault-injection `force` RHS warnings; all ten injection selectors were zero. Strict pinned compilation rejects 36 unproven nested endpoints and has no DV runtime. Full OpenTitan DV and broader §19.5 conformance remain open.

### OT-SYSTEM-FUNCTION-RETURN — `$system` result reaches SystemVerilog

- **Failure:** The pinned released `prim_prince_sim` checked for `dv.stop` through `$system("test -f dv.stop")`; with the file absent, Icarus returned zero to the SV expression and took a false UVM fatal before vector traffic. Paired 2017/2023 reducers confirmed that a nonzero shell status was reported as zero. VVP also warned on the explicitly legal task form.
- **Root cause and fix:** `vpi/sys_icarus.c` set VVP's process exit status instead of the VPI function result. It now returns the C `system()` status through the existing integer-result helper, including `system(NULL)` without an argument. The `$system`-only warning exception in `vvp/vpi_tasks.cc` permits task calls without weakening other system-function diagnostics.
- **Authority and evidence:** IEEE 1800-2017 §20.18.1 and 2023 §20.17.1; [paired RED/GREEN and pinned replay](../../evidence/opentitan-system-return-20260925/README.md). Strict chapter-20 support remains PARTIAL; invalid extra-argument acceptance is DD-062.
- **Released DV boundary:** The unchanged pinned PRINCE smoke with `-gcommercial-unsafe` and native DPI completes five golden plus one random vector and their encryption/decryption comparisons, with one intended pass marker, zero runtime errors/assertions, and normal finish. This is **1/1 named nonstandard compatibility replay**. FuseSoC's three runner-tracked setup warnings (plus a raw backend-deprecation warning) keep the matrix row `DEBT`; the separate merged baseline remains **0/49 clean runtime matrix passes**.
...
- **Boundary:** X/Z bitwise coverpoint samples can incorrectly hit an ordinary bin (DD-063), a separate runtime issue outside this declaration-time scope. Neither a passing language reducer nor a reduced compile-error count is an OpenTitan DV pass.
