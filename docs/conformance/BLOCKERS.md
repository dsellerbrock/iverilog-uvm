...
- **State:** The existing local candidate handles hard distributions whose complete guards are provably active or inactive before drawing; paired 2017/2023 proven-guard focus passes 22/22. The released CSRNG smoke still fails at `dv_base_test.sv:41` when a later random guard remains unresolved. After the named OTP RAM-path overlay, released `otp_ctrl_smoke_vseq` independently reaches the same `cfg.randomize()` failure at 0 ps, with one UVM_FATAL and 512 follow-on assertions; it is 0/1 DV. Paired OTP standalone/nested reducers and a further coupled-child RED reproduce the guard restriction in both editions. A read-only proof review found that prefix-only guard checks are insufficient: complete hard residual factors and later weighted fibers must be validated before any RNG draw. No solver patch or OpenTitan application pass is claimed.
...
- **Evidence:** [Existing candidate and pinned CSRNG replay](../../evidence/opentitan-csrng-proven-guard-replay-20260923/result.json), [paired CSRNG reducer/trace](../../evidence/opentitan-csrng-cfg-joint-dist-20260923/README.md), [focused proven-guard tests](session_logs/2026-09-23_joint_proven_guard_focus.json), [pinned OTP overlay replay with nested cfg reducers](../../evidence/opentitan-otp-backdoor-path-overlay-20260924/README.md), and [paired coupled-child RED](../../evidence/opentitan-joint-ordered-guard-red-20260924/README.md). The OTP compile also has 46 nonfatal `sorry` diagnostics and an ignored constraint item; those remain separate DV qualification gaps.
