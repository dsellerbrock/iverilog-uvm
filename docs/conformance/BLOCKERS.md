...
- **Evidence:** [Paired reducers and pinned replay](../../evidence/opentitan-otp-vif-fixed-input-20260924/README.md). Private ivl `302d7094` passes paired 22/22, registered JSON 13/13, legacy 4/4, neighboring VIF JSON 44/44, negatives 155/155, and real-DPI UVM 358/358. Pinned `otp_ctrl_sim` compiles with zero errors/sorries/assertions. Its released smoke still fails DV at time zero: `mem_bkdr_util.sv:76` rejects an OTP RAM path string with 18 leading spaces; there is no meaningful traffic or pass marker. `chip_sim` remains a compile failure with 48 explicit errors and one sensitivity assertion.
