...
- **State:** The private ordered-guard VVP advances CSRNG past cfg.randomize(), then released CSRNG remains 0/1 DV at 0 ps on a null VIF wait with TL-UL assertion and TEST FAILED CHECKS. The original bytecode omits the object value store for two config-db set calls from class-held parent VIF nested members. A strict paired 2017/2023 reducer fails code generation -3; parent-VIF and direct-child controls pass. The exact loss between expression elaboration and target copy-in emission is not yet fixed.
...
- **State:** After PR #360's class-identity fix, the prior pinned OTP smoke was **0/1 released DV**: `GrantKnown_A` fired in `tb.dut.u_otp_arb` at 1,559,061 ps with known requests/ready, X grant, and all-Z packed `mask_tree`. A legal three-bit disjoint packed-bit assignment tree reproduces the Z value in strict 2017 and 2023; scalar equivalents pass and overlapping variable-bit drivers reject in both modes. The corrected pre-main private compiler now completes the narrow released OTP smoke **1/1 DV** at 132,917,271 ps: one pass marker, zero UVM warnings/errors/fatals or assertion errors, and 2,470 post-sequence scoreboard-routed core TL callbacks. This proves checked smoke traffic, not separate DAI/key-path coverage.
- **Root cause and boundary:** The elaboration-time `collapse_partselect_pv_to_concat` rewrites selected drivers into a whole-vector concat before all assignment RHS connections are complete. The resulting `.part` read of the same full tree creates feedback absent from the source's acyclic bit graph. A pre-fix diagnostic that retains the selected drivers passes the minimal reducer. Simply removing the eager rewrite loses weak/strong resolution through `tran`, because the existing late `cprop` blend uses a nontransparent concat. The corrected late blend uses a transparent concat only for eligible full coverage while retaining its complete-graph dependency guard. Paired focus passes 6/6 on pre-main private ivl `5164a40b` and again in both legacy and JSON harnesses on main-integrated ivl `0f770dc5`; strict 2017/2023 packed-member interaction controls pass. The main-integrated pinned OTP runtime also completes the narrow smoke **1/1 released DV** with the same byte-for-byte runtime log, 2,470 strictly post-start scoreboard callbacks, and zero assertion/UVM errors or fatals. On the pre-SDF private VVP, full legacy passes 6,593 total with zero failures, VPI 131/131, negatives 155/155, runtime invariants 15/15, and real-DPI UVM passes 358/358. The pre-repair full JSON failed 1/3,632 on `br_gh1248`: its valid SDF INTERCONNECT path crosses retained `.part/pv`, resolver, and `.part/sa` nodes and lost 10 ps; the installed baseline passed. A provenance-guarded private VVP `5053e2f4` now passes `br_gh1248` and eight paired 2017/2023 matching/wrong-bit/overlap/extension-port SDF controls 9/9; the six-input extension source is on resolver input 4. Its final pinned OTP replay again qualifies narrow released DV **1/1** at 132,917,271 ps with 2,470 strictly post-start scoreboard callbacks and zero assertion/UVM errors or fatals; the raw runtime log matches pre-SDF byte for byte. Final private legacy passes 6,593 total with zero unexpected failures, VPI 131/131, negatives 155/155, and invariants 15/15; full JSON passes 3,640/3,640 including `br_gh1248` and all eight new SDF controls; real-DPI UVM passes 358/358. This branch is locally qualified for PR publication. A resolver input without structural provenance, including a constant driver on another bit, remains fail-closed for this SDF path; general multi-driver SDF timing is not claimed. The compile still warns about an ignored constraint, so no broader OTP coverage claim follows. Four-state behavior, genuine cycles, strict overlap rejection, pinned source, and all assertions stay intact. CSRNG's nested-VIF value loss is a separate parked blocker.
- **Evidence:** [Paired RED, strength differential, netlist stage probe, focused GREEN, and pinned replay provenance](../../evidence/opentitan-otp-mask-tree-20260924/README.md). Published in [PR #362](https://github.com/dsellerbrock/iverilog-uvm/pull/362).
